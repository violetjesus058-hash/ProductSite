# 待办事项

- [ ] 后续逐个或分批替换商品原始图片链接为稳定托管图片，暂不影响当前目录原型。
- [ ] 继续验证左侧大类导航、搜索、排序和商品卡片点击跳转。
- [ ] 继续验证商品详情页的图片画廊、规格选择、收藏和原始商品链接按钮。
- [ ] 完成桌面端与移动端截图检查，并在交付前保存项目检查点。

- [ ] 将网站展示名称改为原始商品表中的产品名称，落地页供应商标题仅作为辅助采集字段。
- [ ] 检查商品卡片与详情页的名称显示，并保存更新版本。

- [ ] 在左侧大类导航下方增加品牌选择区域，并显示各品牌商品数量。
- [ ] 实现品牌筛选与大类、搜索、排序的组合筛选。
- [ ] 验证桌面侧栏和移动端品牌筛选的可用性，并保存更新版本。

- [ ] 移除首页“Find the next piece.”主视觉横幅。
- [ ] 调整商品工具栏与瀑布流的顶部间距，让商品内容直接进入首屏。
- [ ] 验证首页桌面端与移动端，并保存更新版本。

- [ ] 隐藏左侧大类数量、品牌数量、品牌区总数和结果工具栏数量。
- [ ] 保持类目与品牌筛选逻辑不变，并验证页面显示。
- [ ] 保存隐藏数量后的更新版本。

- [ ] 将右上角用户头像状态替换为 LIVE CATALOG 文案。
- [ ] 验证顶部状态在桌面端和移动端的布局表现。
- [ ] 保存目录状态更新版本。

- [ ] 将右上角状态文案从 LIVE CATALOG 更新为“正在浏览”。
- [ ] 验证顶部文案显示并保存更新版本。

- [ ] 在首页右上角加入“演示数据”标注。
- [ ] 显示 150–300 范围内的随机正在浏览人数和历史访问 2w+。
- [ ] 验证状态布局并保存更新版本。

- [ ] 将顶部“演示数据”文字设置为透明，仅保留浏览人数和历史访问信息。
- [ ] 验证顶部状态布局并保存更新版本。

- [ ] 按当前商品价格筛选更高价的推荐商品。
- [ ] 在详情页底部增加左右箭头横向商品轮播。
- [ ] 验证推荐卡片点击跳转和移动端布局，并保存更新版本。

- [ ] 将推荐商品筛选改为当前价格以上的合理区间随机抽取。
- [ ] 将推荐区改为左图右信息的商品详情模块，包含价格、规格、原始链接和收藏。
- [ ] 验证推荐商品切换与移动端布局，并保存更新版本。

- [ ] 修复详情页颜色、规格和推荐信息列表中的重复 key。
- [ ] 验证指定商品页 /product/7572868073 不再出现控制台 key 警告。
- [ ] 保存错误修复版本。

- [ ] 将推荐区恢复为多个商品卡片横向轮播。
- [ ] 保留合理价格区间随机推荐规则，并支持左右箭头切换。
- [ ] 验证卡片点击跳转和详情页布局，保存更新版本。

- [ ] 将推荐卡片改为与当前商品价格接近的中等客单价商品。
- [ ] 保留左右轮播和点击详情跳转，并验证推荐价格区间。
- [ ] 保存中等客单价推荐更新版本。

- [ ] 移除推荐区顶部标题、说明和固定数量提示。
- [ ] 实现推荐商品左右无限循环选择，不因到达首尾而禁用按钮。
- [ ] 验证无限切换和卡片展示，并保存更新版本。

- [ ] 在少量品牌或品类结果下方显示“正在高标准选择高质量的产品”。
- [ ] 保持商品卡片展示，并验证少量结果页面的留白与移动端布局。
- [ ] 保存选品说明更新版本。

- [ ] 在高质量选品说明下方追加低客单价商品卡片。
- [ ] 让低价商品按换行网格展示，并保留详情页跳转。
- [ ] 验证少量品类页面布局并保存更新版本。

- [ ] 为所有商品卡片增加悬停放大图片效果。
- [ ] 所有少量品类统一显示高标准选品说明与 MORE TO EXPLORE。
- [ ] 将推荐商品改为可持续滑动浏览的循环列表并验证保存。

- [ ] 让品牌+品类组合筛选沿用少量商品说明和 MORE TO EXPLORE。
- [ ] 按同品牌、同品类优先补充低客单价商品。
- [ ] 验证组合筛选页面并保存更新版本。

- [ ] 统一推荐卡片与主商品墙的宽度、比例、圆角和信息排布。
- [ ] 调整推荐区网格和图片展示节奏。
- [ ] 验证两个区域的一致性并保存更新版本。

- [ ] 将选品说明触发条件改为首屏内容不足，而非只按商品数量小于等于 4 判断。
- [ ] 确保不足品类补充 MORE TO EXPLORE 后不会出现空白区域。
- [ ] 验证推荐商品循环滑动无空白并保存更新版本。

- [ ] 建立同品牌、同品类、相关品类和全目录的连续推荐商品池。
- [ ] 实现推荐卡片持续补位，用户浏览时不显示终点或空白。
- [ ] 验证长时间向左/向右浏览并保存更新版本。

- [ ] 检查 ProductSite 仓库是否已存在并确认当前项目 Git 状态。
- [ ] 创建私有 ProductSite 仓库并推送当前商品网站代码。
- [ ] 验证远程仓库内容并返回 GitHub 链接。

- [ ] 将 ProductSite GitHub 仓库可见性改为公开。
- [ ] 验证公开状态并返回仓库链接。

- [ ] 拆分颜色和规格的选择状态。
- [ ] 更新两个选项组的高亮与当前值提示。
- [ ] 验证独立选择并保存修复版本。

- [ ] 将导航、筛选、状态、按钮和空状态文案全部改为英文。
- [ ] 将商品详情、规格、颜色提示和推荐区文案全部改为英文。
- [ ] 检查商品数据中用户可见的中文字段，验证页面纯英文并保存版本。

- [ ] 按全站平均客单价计算 200+ SOLD 演示徽章的显示范围。
- [ ] 为明显低价或高价商品增加 NEW 演示徽章。
- [ ] 验证徽章在主商品墙、MORE TO EXPLORE 和移动端卡片中的位置与可读性。
- [ ] 保存演示徽章更新版本。

- [ ] 为 shoes、fragrance、watches、wallets 类商品增加 CURATED 精选徽章和图标。
- [ ] 处理 CURATED 与 NEW、200+ SOLD 同时出现时的徽章排布。
- [ ] 验证桌面端与移动端商品卡片显示并保存版本。


- [ ] 修复外部部署平台将输出目录误判为 .vitepress/dist 的配置冲突。
- [ ] 保持现有 build 产物 dist/public 与项目部署配置一致。
- [ ] 重新运行生产构建并验证部署输出目录存在。


- [ ] 提交并推送部署输出目录修复到 ProductSite GitHub 仓库。
- [ ] 验证远程分支包含 package.json 和 scripts/sync-deploy-output.mjs 更新。


- [ ] 参考移动端截图优化手机端目录框架，支持部分展示与展开更多。
- [ ] 增加手机端设置入口和页面风格选择：默认、白色、灰色、黑色、浅绿色。
- [ ] 增加字体大小与字体间距三个档位滑轨，默认保持最小值。
- [ ] 验证移动端交互、风格切换和设置持久化并保存版本。


- [ ] 检查预览链接底部未发布提示的来源与正式发布差异。
- [ ] 调整手机端顶部导航、搜索区和商品墙，使其更接近参考图的独立移动框架。
- [ ] 验证 516px 与 375px 宽度下的移动端布局并保存版本。


- [ ] 将最新移动端布局和部署输出修复推送到 ProductSite main 分支。
- [ ] 确认 GitHub main 不再停留在 7756192 旧提交。
- [ ] 重新触发部署并使用最新提交验证构建输出目录。


- [ ] 将手机端类目筛选移动到左侧窄栏，支持点击类目切换。
- [ ] 让右侧顶部只保留搜索框、在线用户数和访问用户数。
- [ ] 让商品图片支持点击放大查看，并保留商品卡片点击进入详情页。
- [ ] 验证手机端类目栏、顶部状态区和图片放大交互并保存版本。


- [ ] 提交并推送手机端左侧类目栏、顶部搜索状态区和图片放大更新。
- [ ] 确认 ProductSite main 分支包含最新手机端代码。


- [ ] 移除手机端 CURATED / 01 与 Explore more 横幅。
- [ ] 验证移除后商品筛选栏和商品卡片上移布局。


- [ ] 移除手机端顶部横向类目和品牌导航。
- [ ] 在左侧类目栏底部加入扩展按钮。
- [ ] 点击扩展后显示当前大类对应的子类列表，并支持筛选选择。
- [ ] 验证子类选择与商品墙联动并保存版本。


- [ ] 将 MORE 面板子类按实际商品数量降序排列。
- [ ] 数量相同的子类使用稳定的字母顺序作为次级排序。
- [ ] 验证排序结果与子类筛选联动。


- [ ] 移除子类列表中的商品数量和商品少提示。
- [ ] 移除 MORE TO EXPLORE 左右箭头和轮播索引状态。
- [ ] 将 MORE TO EXPLORE 改为可持续向下浏览的瀑布流商品墙。


- [ ] 为当前筛选结果建立可循环的随机推荐商品池。
- [ ] 商品墙到达底部时自动追加推荐批次，避免出现空白。
- [ ] 对少量类目和子类验证同类优先、全目录回退的推荐规则。


- [ ] 提交并推送无限随机推荐和连续瀑布流更新。
- [ ] 确认 ProductSite main 分支包含最新推荐逻辑。


- [ ] 进入商品详情页时将页面滚动位置重置到顶部。
- [ ] 将详情页推荐区改为瀑布流布局并移除左右箭头。
- [ ] 为详情页推荐池加入随机循环追加，确保滑动不到空白终点。


- [ ] 删除首页顶部筛选图标和网格视图图标。
- [ ] 验证删除后搜索、收藏、历史记录和在线状态布局正常。


- [ ] 移除商品图片点击放大层，避免点击与详情跳转冲突。
- [ ] 保留并增强商品图片悬停放大细节效果。
- [ ] 验证商品卡片点击统一进入对应落地页。


- [ ] 提交并推送商品卡片悬停放大、点击详情页的交互更新。
- [ ] 确认 ProductSite main 分支包含最新卡片交互代码。


- [ ] 为每个商品生成独立且稳定的随机 Captured 日期。
- [ ] 所有 Captured 日期限制在 2026-08-01 之前。
- [ ] 验证详情页不会统一显示同一天。


- [ ] 为 Ralph Lauren T-shirt 整理 Kakobuy、Oopbuy、Litbuy、Superbuy、GTbuy、Fansbuy 六个平台链接。
- [ ] 区分 GTbuy 与 Superbuy 当前重复链接，避免详情页重复展示。
- [ ] 设计多平台购买入口、平台状态和同款候选标识。


- [ ] 将正确的 Superbuy 链接加入 Ralph Lauren T-shirt 平台清单。
- [ ] 保留 GTbuy 独立入口，不再与 Superbuy 重复。
- [ ] 更新多平台入口方案中的有效平台数量和排序。


- [ ] 将 Kakobuy 标记为 Ralph Lauren T-shirt 的默认主入口。
- [ ] 将 Oopbuy、Litbuy、Superbuy、GTbuy、Fansbuy 作为备用购买来源。
- [ ] 在详情页主按钮和 Buying options 列表中保持该层级。


- [ ] 详情页只保留 Kakobuy 主购买按钮。
- [ ] 隐藏 Oopbuy、Litbuy、Superbuy、GTbuy、Fansbuy 等其他平台入口。
- [ ] 增加中性快速物流提示，不展示其他平台物流对比。


- [ ] 显示 Kakobuy、Oopbuy、Litbuy、Superbuy、GTbuy、Fansbuy 全部平台入口。
- [ ] Kakobuy 作为首位主入口并增加快速物流提示。
- [ ] 其他平台保持独立卡片展示，不使用隐藏式下拉。


- [ ] 修正平台入口显示条件，确保 Ralph Lauren 商品页可见。
- [ ] 将 Oopbuy、Litbuy、Superbuy、GTbuy、Fansbuy 按每行三个按钮排列。
- [ ] 验证平台按钮跳转链接和窄屏布局。


- [ ] 手机壳详情页推荐加入多种配饰和相关品类，避免只重复 Phone Case。
- [ ] 推荐池按图片和品类去重，保持同屏内容有明显差异。
- [ ] 保留无限瀑布流并验证推荐循环不出现单一品类连续重复。


- [ ] 修复推荐循环中索引越界导致的 undefined.images 错误。
- [ ] 为推荐卡片增加无效商品项保护和有效图片回退。
- [ ] 验证 /product/7545217096 及少量推荐池详情页。


- [ ] 所有详情页主按钮统一跳转 Fansbuy。
- [ ] 主按钮文案从 Buy on Kakobuy 改为 Buy on Fansbuy。
- [ ] 平台列表中 Fansbuy 设为主入口并移除 Kakobuy 的主入口高亮。


- [ ] 为每张商品详情图创建独立的产品信息和购买确认区。
- [ ] 每个模块支持对应图片、款式/型号、规格和 Fansbuy 购买入口。
- [ ] 推荐瀑布流放在全部详情图购买模块之后。


- [ ] 在原商品详情首屏之后渲染每张详情图对应的购买确认模块。
- [ ] 每个模块加入商品信息、规格选择和 Buy on Fansbuy 按钮。
- [ ] 确认全部逐图模块之后仍然显示无限推荐瀑布流。


- [ ] 在逐图详情购买模块结束后增加推荐产品标题。
- [ ] 增加简短英文推荐说明，保持与网站 Editorial Pinboard 风格一致。
- [ ] 验证标题与无限推荐瀑布流之间的间距和层级。


- [ ] 不创建或展示虚构的用户评论、国家来源、评分或物流体验。
- [ ] 设计明确标注的 Editorial notes / Catalog observations 卡片。
- [ ] 让观察卡片点击后跳转对应商品详情页，并混排在商品瀑布流中。


- [ ] 将 FIELD NOTE 卡片背景改为纸张主题色。
- [ ] 将卡片标题、说明和按钮文字改为黑色。
- [ ] 保留珊瑚色作为小范围强调，并验证对比度。


- [ ] 详情页推荐瀑布流混入可点击 FIELD NOTE 评论观察卡片。
- [ ] 首页与详情页统一使用 FIELD NOTE 命名、浅色背景和黑色文字。
- [ ] 保持卡片点击跳转对应商品详情页，并避免虚构用户评论表述。


- [ ] 在每张详情图对应的购买模块中保留 Fansbuy 主按钮。
- [ ] 增加 Kakobuy、Oopbuy、Litbuy、Superbuy、GTbuy 等其他平台入口。
- [ ] 验证多平台入口在桌面端和手机端的排列与跳转。


- [ ] 降低首页 FIELD NOTE 卡片插入频率，避免连续出现。
- [ ] 降低详情页推荐瀑布流 FIELD NOTE 插入频率并设置错开位置。
- [ ] 验证评论观察卡片与商品卡片的视觉节奏。


- [ ] 盘点 products.ts 中外部商品图片链接和失效图片数量。
- [ ] 下载并验证可访问的商品主图与详情图。
- [ ] 将稳定图片托管到 webdev 静态资源位置并替换商品数据地址。
- [ ] 验证首页、详情页和推荐瀑布流图片均正常加载。

- [ ] 检查商品详情页当前多平台入口渲染条件。
- [ ] 为普通商品补齐多个平台入口并按每行三个展示。
- [ ] 验证 Fansbuy 主入口、其他平台入口和生产构建。

- [ ] 检查逐图详情模块桌面端宽度、高度和图片比例。
- [ ] 缩小桌面端逐图购买模块，保持左图右信息和三列平台入口。
- [ ] 验证桌面端与移动端详情页并保存版本。

- [ ] 检查逐图图片组件与现有悬浮交互。
- [ ] 为逐图图片实现随鼠标移动的局部放大镜效果。
- [ ] 验证桌面端放大交互、移动端兼容和生产构建。

- [ ] 检查 ITEM NOTES 和商品事实信息区域的顶部间距。
- [ ] 压缩桌面端底部信息区留白并保持移动端可读性。
- [ ] 验证详情页间距、构建并保存版本。

- [ ] 扫描全部商品逐图图片中的电话号码、WhatsApp 和客服联系方式。
- [ ] 从商品图库中移除命中的联系方式图片，并保留安全图片。
- [ ] 验证详情页、缩略图和推荐模块不再展示联系方式图片。


- [ ] 检查 GitHub 仓库中 /manus-storage/ 图片地址是否无法在外部页面访问。
- [ ] 确定 GitHub 可访问的图片托管或发布方案。
- [ ] 修复图片地址、验证首页和详情页，并推送修复版本。


- [ ] 比较免费图片 CDN、GitHub Pages 与仓库托管的限制。
- [ ] 确定适合 3,344 张商品图片的非 Manus 托管方案。
- [ ] 配置 GitHub Pages Actions，并替换图片地址后验证部署。


- [ ] 统计当前商品图片数量和体积，确认 Cloudflare Pages 静态部署预算。
- [ ] 批量转换主图与详情图为 WebP 并控制尺寸和质量。
- [ ] 将 WebP 图片纳入项目静态资源，替换所有 Manus Storage 地址。
- [ ] 验证生产构建、首页、详情页和推荐瀑布流图片加载。


- [ ] 检查商品详情页头部右上角现有布局和分享能力。
- [ ] 增加明显的 Share 按钮、社媒分享菜单和复制产品链接功能。
- [ ] 验证桌面端、移动端以及产品链接分享地址。


- [ ] 评估 Cloudflare Pages 当前部署、域名和可用安全能力。
- [ ] 制定 WAF、速率限制、Bot 防护、热链保护和缓存策略。
- [ ] 加入合理的前端复制提示、图片拖拽限制和版权标识，不影响购买与分享。
- [ ] 验证防护措施不阻断正常商品浏览、社媒分享和平台购买入口。


- [ ] 将 HTML lang、title 和 description 统一为英文。
- [ ] 补齐 Open Graph 和 X/Twitter Card 元标签及英文分享图片信息。
- [ ] 增加 canonical、robots.txt、sitemap.xml 和基础发布日期字段。
- [ ] 验证 Cloudflare Pages 构建输出中的 SEO 文件和英文元信息。


- [ ] 核对样例商品 Fansbuy 页面显示的 USD 与 RMB 价格字段。
- [ ] 盘点 276 个商品的 Fansbuy 链接和当前价格币种。
- [ ] 批量采集可访问平台价格，记录 USD、RMB、采集时间和失败原因。
- [ ] 将确认后的目录展示价格统一为 USD，并保留原始 RMB 校验字段。
- [ ] 验证价格显示、详情页和生产构建，输出异常商品报告。


- [ ] 使用单浏览器会话串行访问商品页面，随机等待并限制访问频率。
- [ ] 读取渲染后的 USD 与 RMB 价格，记录页面状态、时间和失败原因。
- [ ] 只更新有明确页面证据的商品，不使用汇率换算填补缺失值。
- [ ] 价格同步后重新验证目录价格、详情页和生产构建。


- [ ] 用户已确认采用方案 A：分批可视化采集。
- [ ] 当前批次按 10–20 个商品串行读取并保存结果后再继续。


## 最新 Fansbuy SKU 数据表更新

- [ ] Analyze the newly uploaded Fansbuy SKU workbook structure and fields.
- [ ] Compare SKU/product IDs, titles, links, prices, and images against the current 276-product catalog.
- [ ] Define a safe overlap, deduplication, and price-per-SKU update policy before overwriting data.
- [ ] Preserve an import report and rollback copy before applying any catalog update.
- [ ] Run TypeScript and production build validation after the update.



## Kakobuy 商品迁移与图片增量复用

- [ ] Map each existing Fansbuy product to the corresponding Kakobuy product using title, source URL, image evidence, and category.
- [ ] Preserve the current internal product IDs and page routes while adding Kakobuy source IDs and URLs.
- [ ] Build an image fingerprint inventory for existing hosted assets and the new workbook image URLs.
- [ ] Reuse matching hosted images and download only missing or changed images.
- [ ] Define SKU-level price, currency, stock, and variant aggregation rules before updating products.ts.
- [ ] Run a small sample migration before applying the full 276-product update.
- [ ] Validate product pages, image loading, Kakobuy links, TypeScript, and production build.


## Kakobuy 按价格聚合商品目录

- [ ] Count unique products after grouping SKU records by product and USD price.
- [ ] Identify products whose all available variants share one price and can be merged into one record.
- [ ] Identify products with multiple prices that must become separate display records.
- [ ] Define stable IDs, titles, variant labels, stock, links, and image reuse for split records.
- [ ] Prepare a Kakobuy-first catalog rebuild plan that replaces the old 276-product information safely.


## 已确认执行：Kakobuy 价格聚合目录重建

- [ ] Create a rollback snapshot before replacing the current product dataset.
- [ ] Repair the existing products.ts syntax issue before migration.
- [ ] Generate one display record per product and USD price group.
- [ ] Reuse existing hosted images through the image URL manifest.
- [ ] Validate a small sample of migrated Kakobuy product pages and purchase links.
- [ ] Replace the old 276-product dataset with the validated Kakobuy catalog.
- [ ] Run production build and save a checkpoint after successful validation.


## 服饰 Size 选择

- [ ] Inspect Kakobuy variant labels and current detail-page option grouping.
- [ ] Classify apparel variant labels as Size and preserve shoe/accessory specifications as other options.
- [ ] Update generated product records and detail-page labels for apparel size selection.
- [ ] Validate apparel, footwear, and accessory detail pages and save a checkpoint.


## Kakobuy 分类重整

- [ ] Audit raw category, subcategory, title, and variant-label distributions.
- [ ] Detect products whose title indicates pants, shorts, jeans, trousers, or bottoms but are currently mapped to Accessories.
- [ ] Define deterministic precedence rules for clothing, pants, shoes, watches, fragrance, bags, wallets, and other accessories.
- [ ] Rebuild category, subcategory, and brand labels while keeping product IDs, prices, images, and Kakobuy links unchanged.
- [ ] Validate the Pants category, category counts, brand filters, and representative detail pages.


## 逐商品图片与分类复核

- [ ] Locate `High Quality Short Sleeve from Factory` and identify its exact source product and price-group records.
- [ ] Change that product group's primary image to the third gallery image without redownloading assets.
- [ ] Record manual review fields for category, subcategory, brand, primary image index, and review status.
- [ ] Establish a repeatable per-product review queue for future title/image/category corrections.
- [ ] Validate the corrected card and detail page, then save a checkpoint.


## 随机抽样审核模式 (Clothing 类目)
- [x] 禁用无限滚动，改为固定批次展示。
- [x] 实现随机排序并排除已看商品的逻辑。
- [x] 在首页增加“标记本页已看并换一批”的功能。
- [ ] 用户在首页 Clothing 类目下核对当前批次的 40 个商品。
- [ ] 如果发现分类错误或图片问题，记录 SourcePID 并告知 Manus 进行修正。
- [ ] Manus 添加 MANUAL_OVERRIDES 修正逻辑并重新构建。
- [ ] 用户点击“Next Batch”进入下一批随机样本。





## 当前 AI 类目审计（2026-08-16）

- [x] 使用 AI Audit View 以紧凑网格执行 Clothing 批量视觉扫描。
- [x] 对已确认的 Clothing 混入裤装、帽子等商品添加 MANUAL_OVERRIDES 并重建目录。
- [ ] 继续完成 Clothing 剩余批次，避免仅凭标题推断，优先依据主图确认。
- [ ] Clothing 完成后自动切换到下一个尚未完成的类目。
- [ ] 按 Pants、Shoes、Bags、Fragrance、Watches、Accessories 顺序完成全目录审核。
- [ ] 每个类目完成后保存审核日志、统计和构建检查结果。


## AI 审计规则更新

- [ ] Add a review-status field for suspected mismatches that require later confirmation.
- [ ] Keep suspected cases out of MANUAL_OVERRIDES until evidence is conclusive.
- [ ] Continue Clothing with priority on clearly visible pants, hats, and scarves.
- [ ] Verify Clothing completion before switching to Pants.


## 自动更换未审核商品

- [ ] Make AI Audit View choose only unreviewed source products for each batch.
- [ ] Automatically advance to the next unreviewed batch after the current batch is marked complete.
- [ ] Automatically switch to the next category with unreviewed products when the current category is exhausted.
- [ ] Stop advancing and show a completed state when every source product has been reviewed.
- [ ] Preserve suspected cases as reviewable records without treating them as confirmed corrections.


## 连续 AI 审核循环

- [ ] Define a per-batch completion record before advancing to the next batch.
- [ ] Continue Clothing automatically after each verified visual audit batch.
- [ ] Add suspected cases without applying unconfirmed category overrides.
- [ ] Handoff automatically to Pants and subsequent categories after category exhaustion.
- [ ] Stop only after all unique source products have a completed audit record.


## Pants 品类 AI 审计

- [ ] Start the next unreviewed Pants batch in AI Audit View.
- [ ] Validate Jeans, Trousers, Shorts, and Sweatpants subcategories against the primary image.
- [ ] Correct only clearly misclassified Pants records with evidence-based overrides.
- [ ] Mark mixed apparel sets or unclear images as SUSPECTED instead of reclassifying them.
- [ ] Verify Pants completion and continue to the next unreviewed category.


## AI 主导审核模式确认

- [ ] Repair the SUSPECTED_REVIEW generator field format before continuing Pants audit.
- [ ] Use compact high-resolution screenshots for AI-led visual inspection without requiring user confirmation per item.
- [ ] Automatically apply confirmed MANUAL_OVERRIDES, record SUSPECTED cases, and advance to the next unreviewed batch.
- [ ] Continue category handoff after Pants completion.


## 连续 AI 审核持久化恢复

- [ ] Persist active category, current batch, reviewed source IDs, suspected queue, and last audit timestamp.
- [ ] Restore the active category and batch after products.ts regeneration or page reload.
- [ ] Resume from the last incomplete Accessories batch instead of defaulting to Clothing.
- [ ] Verify category handoff and stop only after all unique source products are audited.

## Accessories 断点续审

- [ ] Restore the last Accessories audit session and inspect the current compact batch.
- [ ] Review Accessories images in batches and classify clear caps, scarves, wallets, belts, bags, phone cases, and mixed items.
- [ ] Add only evidence-based manual overrides; keep ambiguous products marked SUSPECTED_REVIEW.
- [ ] Regenerate products, verify audit cursor restoration, and continue the next Accessories batch.

## Accessories 第二批恢复与继续

- [ ] Restore the preview and confirm the second Accessories batch.
- [ ] Mark the already scanned second batch reviewed without re-reviewing the first batch.
- [ ] Continue the next unreviewed Accessories batch and record evidence-based corrections.
- [ ] Verify remaining count and persist the new audit cursor.

## Accessories 继续审核

- [ ] Restore the current Accessories audit breakpoint after the previous preview failure.
- [ ] Mark the already scanned second batch only once and load the next unreviewed batch.
- [ ] Continue visual review and record evidence-based category overrides.
- [ ] Preserve suspected mixed or unclear products without forced category changes.

## Clothing 继续审核

- [ ] Restore the Clothing audit breakpoint and inspect the current compact batch.
- [ ] Review Clothing images for clear Pants, Caps, Scarves, Shoes, Bags, or other cross-category items.
- [ ] Add evidence-based manual overrides and keep ambiguous mixed sets as SUSPECTED_REVIEW.
- [ ] Mark the completed batch, verify remaining count, and continue the next Clothing batch.

## Final catalog preview mode

- [ ] Confirm the final generated product data and manual overrides.
- [ ] Exit AI Audit View and persist normal catalog browsing mode.
- [ ] Verify category navigation, ordinary product grid, and product detail entry.
- [ ] Save a preview checkpoint for the user.

## Accessories second-pass audit

- [ ] Re-audit the entire Accessories category independently of the prior reviewed state.
- [ ] Inspect hats, scarves, wallets, belts, phone cases, electronics, bags, shoes, clothing, and mixed sets.
- [ ] Add only evidence-based category overrides and retain ambiguous products as SUSPECTED_REVIEW.
- [ ] Regenerate the catalog and verify the corrected Accessories category in normal preview mode.
- [ ] Save a second-pass audit checkpoint.


## Remove Brands sidebar module

- [ ] Remove the desktop Brands / All Brands / Unbranded sidebar module.
- [ ] Verify sidebar spacing and mobile layout after removal.
- [ ] Save a checkpoint for the UI change.


## Optimize Kakobuy product titles

- [ ] Analyze source title fields and identify all Catalog Item / ID-only display titles.
- [ ] Build title cleaning and source-field fallback rules without inventing product facts.
- [ ] Regenerate products and measure remaining identifier-style titles.
- [ ] Verify optimized titles on the home grid and product detail pages.
- [ ] Save a checkpoint for the title optimization.


## Product image order correction

- [ ] Locate source product 7576530885 and confirm its four-image gallery.
- [ ] Remove the original first image and promote the original fourth image to the cover.
- [ ] Verify the detail gallery, thumbnails, and product card after regeneration.
- [ ] Save a checkpoint for the image correction.


## Single-product cover image priority

- [ ] Analyze current cover image ordering and identify local gallery images that are multi-product collages.
- [ ] Prefer a clear single-product image whenever one exists in the same gallery.
- [ ] Use a collage or multi-product image only when no single-product image is available.
- [ ] Regenerate and verify representative cards and detail galleries.
- [ ] Save a checkpoint for the cover-image selection update.


## Add purchasing platform links

- [ ] Inventory existing Weidian IDs and current platform link fields.
- [ ] Add only missing Litbuy, GTBuy, Oopbuy, Hipobuy, Fansbuy, LoveGoBuy, Hoobuy, UsFans, AllChinaBuy, Mulebuy, AcBuy, Joyagoo, OrientDig, Sugargoo, BBDBuyEU, VigorBuy, and Fishgoo links.
- [ ] Preserve every existing Kakobuy URL and avoid duplicate products.
- [ ] Validate URL templates, ID substitution, and detail-page platform buttons.
- [ ] Save a checkpoint for the data-only platform link update.

## 本轮：移除非统一口径子类模块

- [ ] 确认移动端子类展开模块及相关状态逻辑
- [ ] 移除不统一口径的子类展开模块
- [ ] 保留已审核的大类导航与正常商品浏览
- [ ] 运行 TypeScript 检查、生产构建并验证移动端
- [ ] 保存稳定检查点并汇报结果

## 本轮：详情页返回位置记忆

- [ ] 检查首页筛选、滚动和详情页返回逻辑
- [ ] 按类目与筛选上下文保存和恢复滚动位置
- [ ] 切换大类时清除新类目继承的旧滚动位置
- [ ] 验证桌面端、移动端和详情页返回行为
- [ ] 保存稳定检查点并汇报结果

## 本轮：修复返回位置记忆失效

- [ ] 复现商品卡片进入详情页后返回的位置恢复失败问题
- [ ] 检查路由切换、首页初始化和滚动恢复时序
- [ ] 重构返回上下文与类目切换滚动控制
- [ ] 实际验证原类目返回和新类目顶部两个流程
- [ ] 保存修复检查点并汇报验证结果

## 本轮：商品卡片主图切换为单品图

- [ ] 定位截图中商品对应的现有商品记录和图片候选
- [ ] 检查并排除拼图、尺码表、集合图和无法确认主体的图片
- [ ] 按单一产品图优先规则更新主图选择或人工覆盖
- [ ] 重新生成商品数据并验证首页卡片、推荐卡片和详情页
- [ ] 保存稳定检查点并汇报调整结果

## 本轮：Shoes 类目单品主图审核

- [ ] 盘点 Shoes 类目商品数量、源商品数量和当前首图
- [ ] 视觉检查鞋类候选图并排除尺码表、拼图和集合图
- [ ] 为有清晰单鞋图的商品写入人工主图覆盖
- [ ] 重新生成数据并验证鞋类卡片与详情画廊
- [ ] 保存稳定检查点并汇报审核结果

## 本轮：当前网站健康检查

- [ ] 检查项目状态、开发服务和构建日志
- [ ] 检查首页、分类导航和商品卡片
- [ ] 检查详情页、返回位置和收藏交互
- [ ] 检查移动端响应式布局与 Shoes 主图更新
- [ ] 汇总已验证项目、问题和后续建议

## 本轮：修复首页标题、移动端溢出与浏览模式文案

- [ ] 定位不完整规格标题、移动端横向溢出和 AI AUDIT VIEW 文案来源
- [ ] 扩展规格标题过滤，保留有效商品名称
- [ ] 将普通浏览状态的 AI AUDIT VIEW 改为清晰目录浏览提示或隐藏
- [ ] 修复 390px 移动端横向布局，保留搜索、分类、排序、收藏和商品卡片功能
- [ ] 构建并验证桌面端、移动端、详情页和返回位置功能
- [ ] 保存修复检查点并汇报结果

## 本轮：隐藏品类具体数量数据

- [ ] 定位品类标题、导航和审核剩余数量的渲染位置
- [ ] 移除所有用户可见的商品数量和 remaining 文本
- [ ] 保留分类筛选、搜索、排序和商品浏览功能
- [ ] 验证桌面端和移动端分类显示
- [ ] 保存稳定检查点并汇报结果

## 本轮：统一商品卡片图片风格

- [ ] 定位灰色区域的图片比例、容器高度和背景样式来源
- [ ] 统一商品卡片图片容器、背景和裁切策略
- [ ] 保留单品主体完整并避免详情页原图受影响
- [ ] 验证桌面端、移动端和商品详情入口
- [ ] 保存稳定检查点并汇报调整结果

## 本轮：指定商品切换单品主图

- [ ] 定位截图商品的精确 SKU、价格和图片候选
- [ ] 视觉审核候选图，仅替换可靠的同商品单品图
- [ ] 无可靠单品图的商品跳过并记录原因
- [ ] 重新生成数据并验证卡片主图和详情画廊
- [ ] 保存稳定检查点并汇报替换与跳过清单

## 本轮：分类体系梳理与优劣评估

- [ ] 读取当前大类、子类字段和分类规则
- [ ] 整理历史采用的分类方式与人工修正方式
- [ ] 统计当前各类商品分布并评估分类风险
- [ ] 汇总分类方案优点、缺点和改进建议

## 本轮：核对当前商品数据规模

- [ ] 统计商品 SKU 总量和唯一源商品数
- [ ] 统计各大类、子类和前台筛选数量
- [ ] 对比网站显示数量、审核状态和数据源数量
- [ ] 汇总当前数据规模及数量差异原因

## 本轮：人工分类错误标记流程

- [ ] 检查当前首页分类筛选、审核排除和本地持久化逻辑
- [ ] 让普通分类浏览显示当前分类下的完整 SKU 数据
- [ ] 在商品卡片增加分类错误标记按钮和已标记状态
- [ ] 保存商品 ID、当前大类、子类、图片和标记时间
- [ ] 验证标记保存、取消、详情跳转和分类切换
- [ ] 保存稳定检查点并说明后续 AI 识图处理流程

## 本轮：修复分类标记丢失

- [ ] 复现连续标记和进入详情页返回后的丢失问题
- [ ] 检查 localStorage 写入时序与首页重新挂载逻辑
- [ ] 修复多商品标记合并保存和恢复
- [ ] 验证标记、取消、详情返回和类目切换
- [ ] 保存修复检查点并汇报结果

## 本轮：已标记商品逐图分类审核

- [ ] 获取并确认当前已标记商品队列
- [ ] 对每个标记商品逐张检查完整图片画廊
- [ ] 记录每张图片的品类证据和不确定点
- [ ] 仅对证据明确的商品写入分类修正
- [ ] 重新生成数据、验证详情页并保存检查点

## 本轮：独立首图分类审计

- [ ] 统计当前商品与各大类首图检查范围
- [ ] 按大类批量检查首图并记录分类证据
- [ ] 仅对明确错误写入分类修正
- [ ] 建立疑似错误复核清单
- [ ] 重新生成数据、验证分类与详情页并保存检查点

## 数量口径审计（2026-08-19）
- [ ] 明确排除 Fansbuy 旧商品的数据识别口径与时间边界。
- [ ] 统计所有现有源工作簿的行数、唯一商品 ID、唯一 Weidian ID 和来源平台字段。
- [ ] 对比源数据、生成后的 SKU、唯一 sourceProductId 和当前前端可见数量。
- [ ] 检查 SKU 按价格合并、相同源商品去重、空字段过滤和分类覆盖造成的数量变化。
- [ ] 检查 Home.tsx 是否默认筛选、审核 seenIds、分类条件或分页逻辑隐藏商品。
- [ ] 输出排除 Fansbuy 后的真实数量拆解、主要原因和可执行修复方案。

## 首页卡片数量核对（2026-08-19）
- [ ] 复现截图中的首页排序、筛选和 Flagged 状态。
- [ ] 统计当前 DOM 实际渲染卡片数，并与 products.ts 和筛选结果对比。
- [ ] 检查 pageSize、slice、AI Audit、收藏/标记状态和本地存储是否导致卡片隐藏。
- [ ] 决定并实现“完整目录浏览”与“审计批次浏览”的清晰数量口径。
- [ ] 验证桌面端和 390px 移动端卡片数量与筛选结果一致。

## ALL PRODUCTS 全量渲染确认（2026-08-19）
- [ ] 确认普通 ALL PRODUCTS 路径不再使用 `slice(0, 40)`。
- [ ] 确认全量首页包含 Clothing、Pants、Shoes、Bags、Fragrance、Watches 和 Accessories/ACC。
- [ ] 用数据总数和 DOM/渲染路径验证 2,192 张展示卡片。
- [ ] 保持 AI Audit View 的 80 张批次限制，不影响普通全量浏览。

## 所有品类全量渲染（2026-08-19）
- [ ] 验证 Clothing、Pants、Shoes、Bags、Fragrance、Watches、ACC 和历史 accessories 普通浏览均不截断。
- [ ] 确认品牌筛选和搜索筛选也显示各自完整结果，而不是前 40 张。
- [ ] 保留 AI Audit View 的批次截断，仅限审核模式生效。
- [ ] 逐类统计卡片数量并完成桌面/移动端验证。

## 收藏清单清空功能（2026-08-19）
- [ ] 检查收藏面板现有状态和持久化逻辑。
- [ ] 增加 Clear all 按钮并清空全部收藏。
- [ ] 保留逐个取消收藏、详情跳转和关闭面板功能。
- [ ] 验证桌面端、移动端和空收藏状态。

## 主图与 Clothing 错类修正（2026-08-20）
- [ ] 为 kb-7778863173-57-70 设置用户提供的 GIF 主图。
- [ ] 核对截图中内衣、袜子、裤装、套装和尺码图对应的 sourceProductId。
- [ ] 将高置信度非上衣商品移出 Clothing，边界商品保留并记录原因。
- [ ] 重新生成数据，验证主图、分类、总量和详情页。

## 服饰空白首图自动修正（2026-08-20）
- [ ] 检查当前 preferred_single_image 与空白/尺码图识别规则。
- [ ] 增加空白图、纯尺码表、联系方式图和拼图过滤。
- [ ] 对服饰画廊优先选择清晰单一商品图。
- [ ] 重新生成并验证截图示例 `kb-7576540787-30-38` 及其他受影响商品。

## 申请上新与反馈（2026-08-20）
- [x] 读取自动化与后端集成规范，确认 Discord 入口和后台回复方案。
- [x] 建立上新申请数据模型、状态和回复字段。
- [x] 升级项目后端能力，加入提交/查询/回复接口骨架。
- [x] 在左侧导航底部增加申请上新入口和中文用户表单。
- [x] 增加后台查看、筛选、回复和状态更新页面。
- [x] 增加 Discord 反馈按钮并验证安全跳转。
- [x] 完成未授权访问防护、构建和基础端到端验证。

## Cloudflare Worker/D1 方案 A（2026-08-20）
- [x] 定义 Worker API、D1 字段和管理员认证占位配置。
- [x] 在当前仓库准备 Worker 源码、wrangler 配置和 D1 迁移 SQL。
- [x] 将申请上新入口接入左侧导航并实现中文表单。
- [x] 增加 Discord 反馈按钮，链接到 https://discord.gg/jtc399kUQV。
- [x] 增加后台申请列表、回复和状态管理页面。
- [x] 在未绑定 Worker 时提供清晰的连接状态和后续配置提示。
- [x] 增加用户申请编号查询页面，展示处理状态和管理员回复。

## Cloudflare 部署错误修复（2026-08-20）
- [x] 将 Cloudflare D1 真实 database_id 写入 `cloudflare-worker/wrangler.toml`。
- [x] 将正确 Pages 来源写入 `ALLOWED_ORIGIN` 配置占位并同步 GitHub。
- [x] 重新触发 Cloudflare Worker 部署并确认 D1 绑定成功。

## 申请后台元数据扩展（2026-08-20）
- [x] 扩展 D1 申请表字段，保存脱敏 IP、国家/地区、设备类型、浏览器、操作系统和 User-Agent 摘要。
- [x] 在 Worker 提交接口中从 Cloudflare 请求头服务端采集元数据，不信任前端传值。
- [x] 在管理员详情页展示完整申请资料、访问元数据和提交/更新时间。
- [x] 保持公开状态查询页不显示 IP、国家、设备等隐私字段。
- [x] 执行 D1 迁移、测试、重新部署 Worker 并验证现有申请记录兼容。

## 申请表单英文与安全增强（2026-08-20）
- [x] 将申请上新表单、申请状态页和管理员页面可见文案统一为自然英文。
- [x] 增加前端输入长度、链接协议、必填字段和提交状态校验。
- [x] 增加 Worker 服务端字段清理、请求体限制、危险链接拦截和基础频率限制。
- [x] 保持公开状态查询页不显示 IP、国家、设备和 User-Agent。
- [x] 完成测试、构建、Cloudflare 部署和英文界面回归验证。


## 2026-08-20 English request flow and security
- [x] Translate the public Request a Product dialog, status lookup, homepage entry, and Discord feedback copy into English.
- [x] Keep the public request response limited to a request ID and status; do not expose IP, country, device, browser, or operating-system metadata to requesters.
- [x] Add server-side request validation, strict length limits, control-character rejection, URL scheme validation, honeypot rejection, and a generic public error response.
- [x] Add rate limiting for public submissions, status lookups, and admin API requests, with no-store responses for sensitive endpoints.
- [x] Expand the admin request view with masked IP, country/region, city, device, browser, operating system, and User-Agent summary fields.
- [x] Run Vitest, TypeScript checks, and the production build successfully.
- [x] Push the change to ProductSite main at commit 97f4151.
- [x] Verify the updated English request flow and admin reply flow against the live Cloudflare Pages/Worker deployment.


## 2026-08-20 tRPC HTML response regression
- [x] Reproduce the homepage API query that receives `<!doctype html>` instead of JSON.
- [x] Identify whether the failure comes from the local server route, Vite fallback, or tRPC client base URL.
- [x] Fix the API routing/client configuration without breaking the existing catalog page.
- [x] Add or update a Vitest regression test for the corrected API path or server behavior.
- [x] Run TypeScript, tests, production build, and homepage browser verification.
- [x] Save a checkpoint after the fix.


## 2026-08-20 Request and Discord entry spacing
- [x] Increase the vertical gap between REQUEST A PRODUCT and DISCORD FEEDBACK in the desktop rail.
- [x] Verify the spacing remains balanced on the mobile layout and save a checkpoint.


## 2026-08-20 Product detail recommendation architecture
- [x] Audit the current CURATED DISCOVERY / Recommended products markup and styles on product detail pages.
- [x] Rework the recommendation header into a clearer editorial hierarchy with a compact eyebrow, title, supporting copy, and exploration cue.
- [x] Separate platform purchase links from the recommendation header so platform buttons do not visually compete with the recommendation title.
- [x] Keep the recommendation product waterfall directly connected to the redesigned header.
- [x] Verify desktop and mobile product detail pages, then save a checkpoint.


## 2026-08-20 Catalog card title refinement
- [x] Audit the current source-title cleaning and card-title display logic against Kakobuy source fields.
- [x] Add concise, evidence-based seasonal, use-case, or recommendation wording without inventing materials, performance, ratings, sales, or reviews.
- [x] Enforce a short-title character budget and remove incomplete size-only or identifier-only titles.
- [x] Verify representative clothing, pants, shoes, accessories, and branded products on desktop and mobile cards.
- [x] Run tests, TypeScript, production build, and save a checkpoint.


## 2026-08-20 Full catalog title audit and repair
- [x] Audit all 2,192 catalog cards and classify identifier-only, SKU-like, size-only, truncated, duplicated, contact-residue, and semantically weak titles.
- [x] Produce before/after title quality statistics by category and problem type.
- [x] Repair every source-determinable title from Kakobuy fields without inventing unsupported product claims.
- [x] Use all available product images and category/subcategory evidence for remaining uncertain titles; keep conservative generic names when visual evidence is insufficient.
- [x] Ensure the example `22543C25` and all similar code-only titles are replaced with readable catalog titles.
- [x] Enforce title length, English wording, and no-contact/no-size-only constraints across all 2,192 cards.
- [x] Verify representative categories and mobile/desktop card rendering, then save a checkpoint with the full statistics.


## 2026-08-20 Three-pass title review: model plus category
- [x] First pass: audit all 2,192 titles for model/code plus `Pants`, `Jeans`, `Shorts`, `Hoodie`, `Jersey`, `Shirt`, `Shoes`, `Bag`, or similar category suffixes.
- [x] Replace model-plus-category titles using Kakobuy source fields, subcategory, and available product-image evidence; do not invent unsupported details.
- [x] Second pass: audit other identifier-like, weak, duplicated, incomplete, and generic titles across every category.
- [x] Third pass: validate title length, English wording, duplicate patterns, price/SKU contamination, and desktop/mobile card display.
- [x] Run tests, TypeScript, production build, full statistics, and save a checkpoint.


## 2026-08-20 Menswear editorial title system
- [x] Define a controlled 20-30 template vocabulary for essential, streetwear, vintage, retro/performance, minimalist, and casual/everyday titles.
- [x] Map category and subcategory terms for menswear, shoes, bags, jerseys, watches, fragrance, and accessories.
- [x] Extract supported title features from Kakobuy source text, category/subcategory, brand, colors, visible graphics, fit words, and product-image evidence.
- [x] Generate concise English titles using `style/scene + product type + key feature`, with a maximum length and no unsupported claims.
- [x] Apply the new title system across all 2,192 cards while preserving confirmed brand, club, team, and product distinctions.
- [x] Run three quality audits for model residue, source-code residue, repetition, unsupported wording, and title length; verify desktop/mobile cards and save a checkpoint.


## 2026-08-20 Broken product image audit
- [x] Audit all 2,192 product image arrays and classify missing, empty, invalid, remote, local, placeholder, and duplicate image sources.
- [x] Identify the affected cards shown as broken images and trace their source product IDs and image URLs.
- [x] Repair invalid/local image references and choose a verified gallery fallback for products with a failed main image.
- [x] Add frontend image-load failure protection so a broken image cannot leave a broken-image icon and alt text as the card visual.
- [x] Run an image quality/statistics audit, verify desktop and 390px mobile cards, run tests/build, and save a checkpoint.


## 2026-08-20 Title system depth diagnosis
- [x] Measure template coverage, unique-title ratio, duplicate-title clusters, generic-title ratio, and category-level title quality across all 2,192 products.
- [x] Measure how often source-supported brand, team/club, color, fit, graphic, season, and subcategory features are actually used in titles.
- [x] Identify where editorial words are generic decoration rather than evidence-based product differentiation.
- [x] Design a stricter category-specific title strategy for tops, hoodies, jackets, pants, shorts, shoes, jerseys, bags, watches, fragrance, and accessories.
- [x] Produce a clear diagnosis and execution plan before applying another full-data title rewrite.


## 2026-08-20 Continue editorial title optimization
- [ ] Build feature-first category rules that prioritize identity, product type, structure, graphic/use, and only then editorial style.
- [ ] Reduce title convergence in Clothing, Pants, and Shoes using source title, subcategory, brand/team, fit, graphic, and image evidence.
- [ ] Extend differentiated naming to Bags, ACC, Watches, Fragrance, and Jerseys without unsupported claims.
- [ ] Run full-catalog uniqueness, generic-title, source-feature usage, code-residue, and fact-boundary audits.
- [ ] Verify desktop/mobile card titles, run tests/build, and save a checkpoint.

- [x] 继续深化 Feature-First 商品标题：读取中文源标题、英文源词和可确认 SKU 类型，减少 Essential Apparel / Essential Trousers 等泛化标题。
- [x] 重新生成 2,192 条商品并恢复持久图片引用，确保标题优化不引入破图或数据数量变化。
- [x] 运行标题深度诊断与全量质量检查，记录唯一标题率、泛化标题数、代码型标题数和分类分布。
- [x] 运行 Vitest、TypeScript 和生产构建，验证本轮标题规则未破坏现有目录和详情页。
- [x] 对 Clothing、Pants、Shoes、Accessories、Watches、Fragrance 代表性商品做最终视觉/语义抽查。
- [x] 视觉模型批量复核暂缓：当前内置视觉 API 余额不足，待恢复额度后再重跑弱标题的多模态复核。

- [x] 在每个品类内容末尾增加 CURATED DISCOVERY 推荐商品模块，保留编辑型标题和说明。
- [x] 将推荐商品改为不等高的瀑布流卡片，适配桌面端和移动端，不强制统一卡片高度。
- [x] 为每个品类建立同类优先、相关品类和全目录回退的持续推荐池。
- [x] 用户向下浏览时持续追加推荐商品，避免品类页推荐模块出现底部空白或终点提示。
- [ ] 验证所有品类切换、推荐商品详情跳转、图片回退、移动端布局和自动追加行为。
- [x] 为品类推荐瀑布流功能补充 Vitest 回归测试并保存项目检查点。

- [x] 修复截图反馈的品类页底部问题：用户滚到底部仍看不到 CURATED DISCOVERY 推荐模块。
- [x] 检查推荐模块是否被空状态、审计模式条件、固定高度或分页结构挡住。
- [x] 将持续推荐触发改为可靠的滚动/底部追加机制，确保用户不会遇到可见终点。
- [ ] 重新验证桌面端截图、移动端滚动、品类切换和推荐商品详情跳转。

- [x] 检查当前仓库分支、远程地址和待提交变更。
- [x] 将已验证的品类推荐滚动修复提交并推送到 GitHub main。
- [x] 确认 GitHub 推送结果并告知 Cloudflare 部署操作，不直接执行发布。

- [x] 排查截图中商品卡片图片整体失效的 URL、代理、CORS 或存储路径根因。
- [x] 修复 SafeProductImage 与商品数据图片地址，使卡片优先加载持久图片并稳定回退。
- [x] 验证首页、品类页、推荐瀑布流和详情页的图片加载与失败占位表现。
- [ ] 验证桌面端和移动端布局，并保存图片修复检查点。

- [x] 检查图片修复版本的 Git 状态、分支和远程地址。
- [x] 提交并推送图片修复版本到 GitHub ProductSite 的 main。
- [x] 核对 GitHub 远程提交，并说明 Cloudflare Pages 部署确认方式。

- [x] 从结果栏移除 CONTINUE EXPLORING 按钮。
- [x] 从结果栏移除 cards/items 产品数量统计，保留品类名称。
- [x] 验证桌面端、移动端、品类切换和推荐区持续滚动不受影响，并保存检查点。
- [x] 明确验证 ALL PRODUCTS、CLOTHING、PANTS、SHOES、BAGS、FRAGRANCE、WATCHES、ACCESSORIES 均不显示数量统计。

- [x] 检查现有收藏、浏览历史和推荐池逻辑，确定兼容的本地行为数据结构。
- [x] 为商品详情页记录停留时长，仅保存本地匿名商品 ID 与时长摘要。
- [x] 将收藏和较长停留商品相关的品类/子类特征加入推荐权重，避免推荐池被单一商品垄断。
- [x] 补充行为权重与推荐排序测试，运行 Vitest、TypeScript 和生产构建。
- [x] 验证品类推荐、详情页推荐和隐私边界，并保存检查点。

- [x] 将 RIVORA 横向商标用于桌面侧栏，将简化 RIVORA icon 用于移动端、详情页和 favicon。
- [x] 更新网站标题、描述、社交元数据和品牌可访问文本为 RIVORA。
- [x] 等待生成资产完成后验证商标/icon 显示，并保存品牌检查点。

- [x] 检查 RIVORA 品牌版本的 Git 分支、远程地址和未提交变更。
- [x] 将 RIVORA 商标/icon 接入版本推送到 GitHub ProductSite main。
- [x] 核对 GitHub main 最新提交并说明 Cloudflare 自动部署状态。

- [x] 检查 RIVORA wordmark 与 icon 资源在预览和生产路径下的实际响应。
- [x] 移除不稳定的异步 wordmark 引用，替换为稳定可部署的本地品牌资产，并保留 RIVORA 识别。
- [x] 验证桌面侧栏、移动头部、详情页和 favicon 不再显示替代文本或破图。
- [x] 保存 RIVORA 品牌资源修复检查点。

- [x] 将桌面侧栏 RIVORA 商标放大至接近分隔线宽度，并保持原始宽高比例。
- [x] 保持移动端与详情页 icon/品牌尺寸不被桌面调整影响。
- [x] 验证桌面和移动端显示并保存商标尺寸调整检查点。

- [x] 检查商标尺寸调整版本的 Git 分支、远程地址和未提交变更。
- [x] 将 RIVORA 商标尺寸调整版本推送到 GitHub ProductSite main。
- [x] 核对 GitHub main 最新提交并说明 Cloudflare 自动部署状态。

- [x] 检查品牌引用为何在线上仍显示替代文本，并定位需要移除的 `/manus-storage` 依赖。
- [x] 将 RIVORA 品牌显示改为不依赖 Cloudflare Pages 外部存储路径的稳定内联标志。
- [x] 在商品卡片增加不喜欢按钮，并将不喜欢商品记录保存在浏览器本地。
- [x] 让首页和详情页推荐模块过滤不喜欢商品，保留收藏和停留时长推荐权重。
- [x] 补充测试并验证桌面端、移动端、推荐过滤和详情页行为，保存检查点。

- [x] 将 RIVORA wordmark、icon 和 favicon 保存到 GitHub 仓库静态目录。
- [x] 将首页、详情页和 HTML favicon 的品牌引用改为仓库相对路径。
- [x] 验证构建产物包含品牌静态文件，并保存检查点。

- [x] 检查仓库静态品牌文件版本的 Git 状态、分支和远程地址。
- [x] 将 client/public/branding 静态品牌文件版本推送到 GitHub ProductSite main。
- [x] 核对 GitHub main 最新提交并说明 Cloudflare 自动部署状态。

- [x] 放大移动端顶部 RIVORA 商标 icon 与文字，提升小屏可读性。
- [x] 调整移动端头部高度和搜索区域，确保 390px 宽度不横向溢出。
- [x] 验证移动端与桌面端响应式显示，并保存商标尺寸检查点。

- [x] 检查主商品、品类商品和推荐商品当前移动端网格规则。
- [x] 将手机端主商品墙、品类商品墙和推荐商品墙统一为两列瀑布流。
- [x] 调整两列卡片间距、图片比例、标题和操作按钮，避免横向溢出。
- [x] 验证 390px 移动端、推荐持续追加和详情跳转，并保存检查点。

- [x] 检查手机端 REQUEST A PRODUCT、DISCORD FEEDBACK 和设置图标的 JSX 与布局关系。
- [x] 将两个入口放回左侧菜单栏延续位置，移除手机端设置/滑杆图标。
- [x] 验证手机端菜单、顶部品牌、搜索框和两列商品墙不再错位，并保存检查点。

- [x] 将手机端左侧 `REQUEST A PRODUCT` 缩写为 `REQUEST`，将 `DISCORD FEEDBACK` 缩写为 `DISCORD`。
- [x] 保留完整 aria-label/title 和原有点击、外链功能，验证缩写后菜单不再多行换行。
- [x] 运行移动端截图、测试和构建，并保存检查点。

- [x] 检查手机端搜索框、品牌区、状态区和左侧菜单当前对齐规则。
- [x] 统一搜索框水平垂直居中，以及左侧菜单文案、图标、分隔线的居中格式。
- [x] 验证 390px 手机端搜索、菜单和两列商品墙不溢出，并保存检查点。

- [x] 检查手机端居中调整版本的 Git 分支、远程地址和未提交变更。
- [x] 将搜索框和左侧菜单居中调整版本推送到 GitHub ProductSite main。
- [x] 核对 GitHub main 最新提交并说明 Cloudflare 自动部署状态。

- [x] 收窄手机端左侧分类栏，避免占用过多商品展示宽度。
- [x] 增大并加粗手机端分类、收藏、REQUEST 和 DISCORD 文案，保持统一层级。
- [x] 验证移动端侧栏、搜索区和两列商品墙无横向溢出，并保存检查点。

- [x] 检查手机端侧栏优化版本的 Git 分支、远程地址和未提交变更。
- [x] 将手机端侧栏收窄、字体放大加粗版本推送到 GitHub ProductSite main。
- [x] 核对 GitHub main 最新提交并说明 Cloudflare 自动部署状态。

- [x] 将手机端顶部改为小尺寸 RIVORA 图标、居中搜索框、右侧在线与历史访问信息三段式布局。
- [x] 缩短手机端顶部高度，移除多余收藏/历史图标占位并保持状态信息可读。
- [x] 验证手机端顶部与左侧 rail、两列商品墙衔接无溢出，并保存检查点。

- [x] 检查逐图详情模块、平台入口列表和下一模块的布局高度与溢出关系。
- [x] 修复逐图模块之间的遮挡，确保每个模块完整收束并保留合理间距。
- [x] 验证多图帽子等商品详情页、移动端详情页和推荐区，并保存检查点。

- [x] 二次复现并测量多平台详情模块的实际覆盖范围。
- [x] 增加 grid 行高、模块最小高度、信息区溢出和层叠隔离防护。
- [x] 二次回归验证 Essential Cap 桌面端与 390px 移动端详情页。

- [x] 检查详情页左右切换按钮、缩略图和主图状态的通用事件链。
- [x] 修复所有商品详情页的图片切换、序号更新和缩略图选中状态同步。
- [x] 验证多个多图商品的桌面端与移动端画廊交互，并保存检查点。

- [x] 检查图片切换修复版本的 Git 分支、远程地址和未提交变更。
- [x] 将图片切换修复版本推送到 GitHub ProductSite main。
- [x] 核对远程 main 最新提交并说明 Cloudflare 自动部署状态。

- [x] 将手机端在线人数与历史访问人数从搜索框右侧移至搜索框正下方。
- [x] 保持第一行小图标与搜索框对齐，第二行状态信息并列且清晰可读。
- [x] 验证 390px 手机端顶部和商品区域无溢出，并保存检查点。

- [x] 放大手机端顶部左侧 RIVORA 图标，保持图标不挤压搜索框。
- [x] 验证放大后的图标与双行状态 header 垂直居中、390px 无溢出，并保存检查点。

- [x] 检查手机端图标放大版本的 Git 分支、远程地址和未提交变更。
- [x] 将手机端 RIVORA 图标放大版本推送到 GitHub ProductSite main。
- [x] 核对 GitHub main 最新提交并说明 Cloudflare 自动部署状态。

- [x] 检查首页搜索/筛选无结果分支与现有推荐数据源。
- [x] 在无结果状态下增加桌面端和手机端推荐商品瀑布流，并支持继续浏览。
- [x] 验证空结果页面的推荐内容、清除筛选按钮和响应式布局，并保存检查点。

- [x] 检查无结果推荐模块版本的 Git 分支、远程地址和未提交变更。
- [x] 将无结果推荐模块版本推送到 GitHub ProductSite main。
- [x] 核对 GitHub main 最新提交并说明 Cloudflare 自动部署状态。

- [x] 检查无结果推荐模块当前列数、sentinel 和追加触发逻辑。
- [x] 将手机端无结果推荐固定为两列，桌面端保持响应式瀑布流。
- [x] 让无结果推荐持续追加、循环推荐池且无终点，并完成双端验证。

- [x] 检查无结果推荐池和循环渲染造成重复商品的原因。
- [x] 按 sourceProductId / 商品 ID 去重推荐池，追加时优先展示未出现过的商品并避免连续重复。
- [x] 验证两列推荐瀑布流的重复控制与持续浏览行为，并保存检查点。

- [x] 检查推荐去重修复版本的 Git 分支、远程地址和未提交变更。
- [x] 将推荐去重修复版本推送到 GitHub ProductSite main。
- [x] 核对 GitHub main 最新提交并说明 Cloudflare 自动部署状态。

- [x] 检查商品详情页现有风险提示位置和英文文案。
- [x] 将平台名称替换为 ProductSite，并优化第三方商品、采购服务和风险承担说明。
- [x] 在每个商品详情页加入统一免责声明，验证桌面端和手机端显示并保存检查点。

- [x] 检查现有分析 endpoint、网站统计脚本和客户端事件基础。
- [x] 设计匿名事件模型、会话标识与 UTM 来源归因，不采集姓名、邮箱或原始 IP。
- [x] 接入页面、分类、搜索、商品曝光/点击/预览、详情浏览、平台出站和无结果搜索事件。
- [x] 编写埋点单元测试，验证事件参数、去重和隐私边界。
- [x] 在桌面端和手机端验证关键事件触发，并完成构建与检查点。

- [ ] 比较独立自有埋点面板、现有项目后台扩展和第三方自托管分析三种方案。
- [ ] 确定事件接收接口、D1 存储字段、数据保留边界和管理员访问控制方式。
- [ ] 实现自有埋点写入接口与管理员数据面板，展示事件、来源、商品和平台点击汇总。
- [ ] 验证匿名事件入库、管理员权限、限流/参数校验和报表查询，并保存检查点。

- [x] 检查现有 Worker 路由、D1 绑定、ADMIN_API_KEY 校验和前端 Worker client。
- [x] 设计 analytics_events 表、匿名字段边界、批量写入接口和管理员汇总接口。
- [x] 实现 Worker 埋点接收、管理员查询、基础限流和参数校验。
- [x] 在 ProductSite 增加 /admin/analytics 管理面板与汇总图表/表格。
- [x] 编写接口与面板测试，验证事件字段边界、匿名配置和响应式展示。

- [ ] 在 Cloudflare D1 的 productsite-db 执行 migrations/003_analytics_events.sql，并重新部署 Worker 后验证真实事件入库。

- [ ] 检查自有分析面板版本的 Git 分支、远程地址和未提交变更。
- [ ] 将自有埋点与 /admin/analytics 版本推送到 GitHub ProductSite main。
- [ ] 核对 Cloudflare Pages 构建完成后线上 /admin/analytics 页面可访问。

- [ ] 为 /admin/analytics 的 Worker admin key 输入框增加显示/隐藏按钮。
- [ ] 排查 Failed to fetch 的 Worker URL、CORS、ADMIN_API_KEY 和线上 Worker 版本。
- [ ] 验证密钥控件与真实汇总接口请求，并保存检查点。

- [ ] 将分析面板密钥显示/隐藏、Worker CORS 和错误提示修复推送到 GitHub ProductSite main。
- [ ] 重新部署 Worker 与 Pages，并验证新 ADMIN_API_KEY 可以加载 D1 汇总数据。

- [x] 盘点现有匿名埋点字段与当前面板未展示的行为维度。
- [x] 扩展 Worker 汇总接口，增加来源、页面、设备、语言、搜索、无结果、收藏、停留和平台转化统计。
- [x] 将分析面板标题、筛选器、指标、排行、表格和提示全部改为中文。
- [x] 验证各统计字段与事件参数对应，完成桌面端和手机端回归并保存检查点。

- [x] 修复 Worker 最近事件明细未按所选统计周期过滤的问题。
- [x] 处理生产构建中的 Umami 环境变量占位符，确保不再输出未替换的脚本地址。
- [x] 回归分析面板中文显示、Worker CORS/权限、D1 汇总和桌面移动端布局。
- [x] 保存并推送上线优化版本，整理 Cloudflare 分别部署步骤。

- [x] 将分析周期改为今天、昨天、最近 7 天、最近 30 天。
- [x] Worker 按北京时间自然日计算今天和昨天，并保持 7/30 天周期准确。
- [x] 验证四种周期的面板显示、趋势和最近事件数据。

- [x] 规范推广链接 UTM 参数并记录 YouTube、TikTok、Instagram、Discord 等渠道示例。
- [x] 检查并完善 UTM 来源、媒介、活动的会话持久化与 Worker 入库。
- [x] 在中文分析面板增加来源/媒介/活动归因维度和清晰的渠道展示。
- [x] 测试带参数链接无感进入网站、跨页面归因保持和后台统计对应。

- [x] 新增受保护的推广链接生成器页面和路由。
- [x] 支持自定义渠道、媒介、活动、投放位置及首页/商品目标路径。
- [x] 自动编码并生成可复制的 UTM 链接，保留无感进入网站的行为。
- [x] 验证管理员密钥保护、链接参数正确性、复制交互和移动端布局。

- [x] 将推广链接生成器流量来源改为固定下拉列表，包含用户指定的 20 个来源。
- [x] 验证来源选择后生成的 utm_source 值、页面显示和生产构建。

- [x] 在来源下拉中增加“自定义网址”选项，并保留现有 20 个固定平台。
- [x] 对自定义网址校验协议、提取主机名并生成稳定的 utm_source/utm_content。
- [x] 验证网址来源链接编码、后台归因字段和移动端表单显示。

- [x] 盘点构建脚本、运行时导入和环境变量中的 Manus 依赖。
- [x] 核对 Pages、Worker、D1、图片资源和外部 API 是否可独立运行。
- [x] 修复会阻断 Cloudflare 独立部署的必要依赖，并补充独立部署说明。
- [x] 验证脱离 Manus 后的本地构建、Worker 构建和前端运行路径。

- [x] 检查管理员回复保存后用户端状态页面的显示链路。
- [x] 修复 AdminRequests 与 Worker 管理接口返回字段命名不一致，确保回复可编辑、保存和回读。
- [x] 增加管理员回复/状态更新回归测试，并验证用户状态查询显示。

- [x] 在 Request a Product 弹窗加入简明英文提交流程说明。
- [x] 验证流程说明在桌面端和移动端不会遮挡表单或操作按钮。

- [x] 新增受保护的批量产品上传后台路径。
- [x] 提供可下载的产品数据模板并说明字段填写规则。
- [x] 支持上传后解析、字段校验、重复识别和更新预览。
- [x] 仅在管理员确认后写入产品数据，禁止未确认的破坏性覆盖。
- [x] 验证大批量文件、错误提示、权限保护和移动端后台布局。

- [x] 采用 CSV 模板作为批量上传格式。
- [x] 设计商品字段与新增/更新/跳过规则，禁止未确认覆盖。
- [x] 为 Cloudflare Worker/D1 增加商品持久化接口和迁移。
- [x] 创建受保护的 CSV 上传后台、模板下载和变更预览。
- [x] 让前台商品读取兼容 D1 更新数据，并完成端到端验证。

- [x] CSV 模板增加一行来自现有商品目录的真实示例数据。
- [x] 增加管理员导出当前完整商品数据的 CSV 功能。
- [x] 验证导出字段、重新上传兼容性和移动端操作布局。

- [x] 批量上传后台在下载模板、导出、选择上传和预览前统一验证管理员密钥。
- [x] 未验证时锁定全部操作，验证成功后解锁并保留 Worker 服务端鉴权。
- [x] 验证权限门禁、错误提示和移动端布局。

- [x] CSV 仅保留一个来源商品链接即可完成导入。
- [x] 识别 Kakobuy、Fansbuy 及 Weidian 链接并提取 Weidian 商品 ID。
- [x] 根据 Weidian ID 自动生成缺失平台链接，保留已有链接和原 Kakobuy 链接。
- [x] 在导入预览中显示识别结果和将要生成的平台链接，并补充回归测试。

- [x] 移除商品卡片上的品类错误标记按钮、提示和相关交互状态。
- [x] 检查首页、详情页、Request a Product、管理员申请、分析、推广链接和 CSV 导入导出路径。
- [x] 检查 Worker/D1 接口权限、CORS、错误响应、环境变量和独立 Cloudflare 部署条件。
- [x] 完成类型检查、单元测试、生产构建、桌面端和 390px 移动端验证。

- [x] 手机端左侧菜单爱心打开专用收藏视图，仅显示已收藏产品。
- [x] 收藏视图在手机端使用两列瀑布流卡片，并保留收藏/不喜欢操作。
- [x] 收藏列表下方增加推荐产品模块，排除不喜欢和已收藏商品并支持继续浏览。
- [x] 验证爱心入口、清空收藏、详情返回和 390px 移动端布局。

- [x] 收藏视图打开时取消当前分类高亮并禁止分类菜单切换。
- [x] 退出收藏视图后恢复分类菜单和正常商品浏览。
- [x] 验证手机端收藏模式不再出现多选分类的视觉和交互冲突。

- [x] 编写中文 ProductSite 网站管理指南，包含功能路径、使用规则和部署流程。
- [x] 将指南保存为项目 Markdown 文件并交付给用户。

- [ ] 为浏览历史面板增加手机端独立滚动区域和最大高度。
- [x] 保持历史面板标题、关闭按钮和历史条目可用，验证底部记录可滑动查看。
- [x] 完成历史面板移动端回归测试并保存版本。

- [x] 桌面端和手机端浏览历史面板都增加独立滚动区域及最大高度限制。
- [x] 保持标题和关闭按钮可见，验证全部历史记录可以滚动查看。

- [x] 为手机端分类切换和高频按钮增加点击节流，忽略短时间重复操作。
- [x] 合并筛选、滚动恢复、推荐刷新和埋点触发，减少连续点击造成的重复渲染。
- [x] 验证快速切换分类、收藏/不喜欢、历史和推荐加载不会导致页面卡住。

- [x] 在 Worker 分析汇总中增加按渠道去重用户数统计。
- [x] 在中文分析面板增加今天、昨天、最近 7 天、最近 30 天的渠道用户统计展示。
- [x] 为渠道统计补充接口/数据聚合测试，并验证桌面端与手机端显示。

- [x] 将最近事件明细改为分页展示，避免一次性无限向下堆叠。
- [x] 增加每页 10、50、100、500 条选择，并支持页码跳转。
- [x] 显示当前页范围、总记录数并验证桌面端与手机端分页布局。

- [x] 为分析卡片设计独立统计周期选择状态与查询参数。
- [x] 扩展 Worker 支持今天、昨天、本周、过去 7 天、过去 14 天、本月、过去 30 天、所有时间及自定义日期。
- [x] 在渠道用户数及其他主要分析模块中展示独立周期控件和对应日期范围。
- [x] 完成自定义日期选择、响应式布局和统计回归验证。
