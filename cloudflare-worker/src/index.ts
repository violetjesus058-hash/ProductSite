...
export function analyticsRowInput(input: AnalyticsInput) {
  ...
  return {
    value: {
      ...
      query: analyticsText(input.query ?? input.search_term ?? input.searchTerm, 160),
...
async function getAnalyticsSummary(request: Request, env: Env) {
  ...
  const requestedSearchPage = Number(url.searchParams.get("searchPage") || "1");
  const requestedSearchPageSize = Number(url.searchParams.get("searchPageSize") || "25");
  const searchPage = Number.isInteger(requestedSearchPage) && requestedSearchPage > 0 ? requestedSearchPage : 1;
  const searchPageSize = [10, 25, 50, 100].includes(requestedSearchPageSize) ? requestedSearchPageSize : 25;
  ...
  const [totals, events, products, platforms, categories, daily, sources, sourceUsers, media, campaigns, devices, languages, paths, searches, searchTotal, conversions, recentTotal, recent] = await Promise.all([
    ...
    env.DB.prepare("SELECT query AS name, COUNT(*) AS count, SUM(CASE WHEN event_name = 'search_no_result' THEN 1 ELSE 0 END) AS no_result_count FROM analytics_events WHERE occurred_at >= ? AND occurred_at < ? AND event_name IN ('search', 'search_no_result') AND query IS NOT NULL AND query != '' GROUP BY query ORDER BY count DESC, name ASC LIMIT ? OFFSET ?").bind(since, until, searchPageSize, searchOffset).all(),
    env.DB.prepare("SELECT COUNT(*) AS count FROM (SELECT query FROM analytics_events WHERE occurred_at >= ? AND occurred_at < ? AND event_name IN ('search', 'search_no_result') AND query IS NOT NULL AND query != '' GROUP BY query)").bind(since, until).first(),
  ...
  return response(request, env, {
    days, range, periodLabel, page, pageSize, searchPage, searchPageSize, searchTotal: Number((searchTotal as { count?: number } | null)?.count || 0), recentTotal: Number((recentTotal as { count?: number } | null)?.count || 0),
...
const affiliateTemplates: Record<string, (id: string) => string> = {
  Litbuy: (id) => `https://litbuy.com/product/weidian/${id}?inviteCode=XXGYH4Z80`, GTBuy: (id) => `https://gtbuy.com/product/weidian/${id}?inviteCode=XO78PVRZW`, Oopbuy: (id) => `https://oopbuy.com/product/weidian/${id}?inviteCode=Y5DH4UF2W`, Hipobuy: (id) => `https://hipobuy.com/product/weidian/${id}?inviteCode=P6PP29VX7`, Fansbuy: (id) => `https://fansbuy.com/item-micro-${id}.html?promotionCode=R0dfTU9DRzA2VTk`, LoveGoBuy: (id) => `https://www.lovegobuy.com/product?shop_type=weidian&id=${id}&invite_code=U577HX`, Hoobuy: (id) => `https://hoobuy.com/product/2/${id}?inviteCode=K8l2grxX`, UsFans: (id) => `https://www.usfans.com/product/3/${id}?ref=BCSLQC`, AllChinaBuy: (id) => `https://www.allchinabuy.com/en/page/buy/?nTag=Home-search&from=search-input&_search=url&position=&url=https%3A%2F%2Fweidian.com%2Fitem.html%3FitemID%3D${id}&partnercode=EEa5go`, Mulebuy: (id) => `https://mulebuy.com/product/?shop_type=weidian&id=${id}&ref=200209428`, AcBuy: (id) => `https://www.acbuy.com/product?id=${id}&source=WD&u=3FLN4S`, Joyagoo: (id) => `https://joyagoo.com/product?platform=WEIDIAN&id=${id}&ref=300950678`, OrientDig: (id) => `https://orientdig.com/product/?shop_type=weidian&id=${id}&ref=100245718`, Sugargoo: (id) => `https://www.sugargoo.com/products?productLink=${encodeURIComponent(`https://weidian.com/item.html?itemID=${id}`)}&memberId=3229305473717352480`, BBDBuyEU: (id) => `https://www.bbdbuyeu.com/goods/WEIDIAN/${id}?inviteCode=j4zwj7`, VigorBuy: (id) => `https://vigorbuy.com/product/2/${id}?inviteCode=jkNlpqAP`, Fishgoo: (id) => `https://www.fishgoo.com/#/product?productLink=${encodeURIComponent(`https://weidian.com/item.html?itemID=${id}`)}&memberId=TG2665ux5KieI`,
};
export function extractWeidianId(value: string) {
  if (!validUrl(value)) return "";
  try { const url = new URL(value); const fromQuery = url.searchParams.get("itemID") || url.searchParams.get("id"); if (fromQuery && /^\d{6,20}$/.test(fromQuery)) return fromQuery; const match = `${url.hostname}${url.pathname}`.match(/(?:item-micro-|goods\/WEIDIAN\/|product\/(?:2|3)\/|product\/weidian\/|weidian\/|itemID[=/_-])([0-9]{6,20})/i); return match?.[1] || ""; } catch { return ""; }
}
export function generateAffiliateLinks(sourceUrl: string, existing: Record<string, unknown> = {}) {
  const id = extractWeidianId(sourceUrl); if (!id) return { id: "", links: existing };
  const links: Record<string, string> = { ...Object.fromEntries(Object.entries(affiliateTemplates).map(([name, create]) => [name, create(id)])) };
  for (const [name, value] of Object.entries(existing)) if (typeof value === "string" && value.trim()) links[name] = value;
  return { id, links };
}
...
export function normalizeProductImportRow(input: unknown) {
  ...
  const sourceUrl = normalizeText(row.url, 1000); const existingPlatformLinks = objectValue(row.platformLinks); const generated = generateAffiliateLinks(sourceUrl, existingPlatformLinks);
  const payload = { id, sourceProductId: normalizeText(row.sourceProductId, 120) || generated.id, name, catalogName: normalizeText(row.catalogName, 240) || name, category, subCategory: normalizeText(row.subCategory, 120), brand: normalizeText(row.brand, 120), price, referencePrice: row.referencePrice === "" || row.referencePrice == null ? null : Number(row.referencePrice), currency: normalizeText(row.currency, 12) || "USD", description: normalizeText(row.description, 2000), sizes: arrayValue(row.sizes), colors: arrayValue(row.colors), stock: normalizeText(row.stock, 80), shop: normalizeText(row.shop, 160), shopUrl: normalizeText(row.shopUrl, 500), url: sourceUrl, images: arrayValue(row.images), tags: arrayValue(row.tags), collectedAt: normalizeText(row.collectedAt, 80) || new Date().toISOString(), reviewStatus: normalizeText(row.reviewStatus, 20) || "unreviewed", reviewNote: normalizeText(row.reviewNote, 500), platformLinks: generated.links };
