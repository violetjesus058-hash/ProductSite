...
export default function ProductDetail() {
  ...
  return <div className="detail-shell">
    ...
    {recommended.length > 0 && <section className="recommendations"><div className="recommendation-waterfall">{Array.from({ length: recommendationCount }, (_, index) => { const item = recommended.length > 0 ? recommended[index % recommended.length] : null; if (!item) return null; const title = englishValue(cleanTitle(item.catalogName || item.name), `Catalog Item ${item.id}`); const note = item.subCategory ? `Observed format: ${englishValue(item.subCategory, "catalog format")}. Review the product page for the listed options.` : "Catalog observation: review the product page for the listed options and details."; return <Fragment key={`${item.id}-${index}`}><Link href={`/product/${item.id}`} className="recommendation-card"><div className="recommendation-image"><img src={item.images[0] || "/manus-storage/catalog-detail-stilllife_f1f3f213.jpg"} alt={title} loading="lazy" /></div><div className="recommendation-meta"><strong>{title}</strong><span>{money(item.price, item.currency)}</span></div></Link>{(index + 1) % 6 === 0 && <Link href={`/product/${item.id}`} className="editorial-note-card recommendation-note-card"><div className="editorial-note-mark">FIELD NOTE</div><div className="editorial-note-title">{title}</div><p>{note}</p><span>Open product <ArrowUpRight size={13} /></span></Link>}</Fragment>; })}</div><div ref={recommendationSentinel} className="recommendation-sentinel" aria-hidden="true" /></section>}
