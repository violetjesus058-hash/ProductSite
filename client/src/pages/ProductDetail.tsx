...
function platformIconFor(name: string) {
  ...
  return platformIconFiles[key] ? `/platform-icons/${platformIconFiles[key]}` : "";
}
function PlatformIdentity({ source }: { source: PlatformSource }) {
  const icon = platformIconFor(source.name);
  return icon ? <img src={icon} alt="" aria-hidden="true" loading="lazy" /> : <span className="platform-source-fallback" aria-hidden="true">{source.name.charAt(0).toUpperCase()}</span>;
}
...
export default function ProductDetail() {
  ...
  const relatedStyles = useMemo(() => {
    if (!product?.sourceProductId) return product ? [product] : [];
    return products.filter((item) => item.sourceProductId === product.sourceProductId).sort((a, b) => a.price - b.price || a.id.localeCompare(b.id));
  }, [product]);
  ...
  useEffect(() => {
    if (!product) return;
    trackEvent("product_detail_view", { product_id: product.id, product_name: cleanTitle(product.catalogName || product.name).slice(0, 120), category: product.category, price: product.price, currency: product.currency, image_count: gallery.length });
  }, [product?.id]);
  ...
  const trackAffiliateClick = (source: PlatformSource, location: string) => {
    if (!product) return;
    trackEvent("affiliate_click", { product_id: product.id, product_name: cleanTitle(product.catalogName || product.name).slice(0, 120), category: product.category, price: product.price, currency: product.currency, platform: source.name, destination: source.name, location });
  };
  ...
  return <div className="detail-shell">
    ...
    <main className="detail-content">
      ...
      <section className="detail-dossier"><div className="dossier-eyebrow">FROM THE CATALOG <span>/{englishCategoryLabels[product.category] || product.category.toUpperCase()}</span></div><div className="dossier-heading"><h1>{englishValue(cleanTitle(product.catalogName || product.name), `Catalog Item ${product.id}`)}</h1><span className="dossier-id">#{product.id}</span></div>{styleLabelFor(product) && <div className="dossier-style-label">{styleLabelFor(product)} · captured price option</div>}<div className="dossier-price"><strong>{money(product.price, product.currency)}</strong>{product.referencePrice && product.referencePrice > product.price && <del>{money(product.referencePrice, product.currency)}</del>}<span>{product.currency}</span></div><div className="dossier-status"><span className="status-dot" /> {englishValue(product.stock || "Availability pending", "Availability pending")}<span className="status-sep">·</span> {englishValue(product.shop || "Store", "Store")}</div>
        ...
        {relatedStyles.length > 1 && <section className="related-styles" aria-label="Other styles from this product"><div className="related-styles-head"><div><span>OTHER STYLES</span><p>Each card represents a separate style and its reference price.</p></div><small>{relatedStyles.length} styles</small></div><div className="related-style-grid">{relatedStyles.map((item) => { const isCurrent = item.id === product.id; const itemStyle = styleLabelFor(item) || "Style option"; return <Link key={item.id} href={`/product/${item.id}`} className={`related-style-card ${isCurrent ? "is-current" : ""}`} aria-current={isCurrent ? "page" : undefined}><div className="related-style-thumb"><SafeProductImage sources={item.images} alt={`${itemStyle} preview`} loading="lazy" /></div><div className="related-style-content"><strong>{itemStyle}</strong><span>{money(item.price, item.currency)}</span><small>{isCurrent ? "CURRENT STYLE" : "VIEW STYLE"}</small></div><ArrowUpRight size={14} /></Link>; })}</div><p className="related-styles-note">All styles use the same source product link. Confirm the selected style, size, price, and availability on the purchasing platform.</p></section>}
        <div className="dossier-actions"><a className="primary-action" href={primarySourceFor(product).url} target="_blank" rel="noreferrer" onClick={() => trackAffiliateClick(primarySourceFor(product), "detail_primary_action")}><ShoppingBag size={17} /> Buy on {primarySourceFor(product).name} <ArrowUpRight size={15} /></a><button className="secondary-action" onClick={toggleLiked}><Heart size={16} fill={liked ? "currentColor" : "none"} /> {liked ? "Saved" : "Save"}</button></div>
        {hasPlatformSources && <section className="platform-sources" aria-label="Buying platforms"><div className="platform-sources-head"><span>BUYING PLATFORMS</span><small>Choose a source</small></div><div className="platform-source-list">{platformSources.map((source) => <a key={source.name} className={`platform-source ${source.primary ? "is-primary" : ""}`} href={source.url} target="_blank" rel="noreferrer" onClick={() => trackAffiliateClick(source, "platform_grid")} ><span className="platform-source-identity"><PlatformIdentity source={source} /><strong>{source.name}</strong></span><span className="platform-source-arrow"><small>{source.primary ? "Fast logistics" : "Open source"}</small><ArrowUpRight size={14} /></span></a>)}</div></section>}
    ...
    <aside className="detail-disclaimer" aria-label="Product and purchasing disclaimer"><div className="detail-disclaimer-label">PRODUCTSITE / IMPORTANT NOTICE</div><p><strong>ProductSite (productsite-8wf.pages.dev)</strong> provides product information and purchasing assistance based on links and data supplied by third-party platforms. ProductSite does not manufacture, inspect, authenticate, or guarantee any product. Product images, descriptions, prices, availability, shipping times, and options may change and should be confirmed with the selected purchasing platform before ordering.</p><p>By continuing through a purchasing link, you acknowledge that the order is placed through the selected third-party platform and that you accept the risks related to product quality, authenticity, fulfillment, and delivery. For questions about an order, please contact the support team of the purchasing platform you selected.</p></aside>
    <section className="detail-purchase-modules" aria-label="Product detail selections">{gallery.map((image, index) => <article className="detail-purchase-module" key={`${image}-${index}`}><ZoomableDetailImage image={image} alt={`${englishValue(cleanTitle(product.catalogName || product.name), `Catalog Item ${product.id}`)} detail ${index + 1}`} label={`${String(index + 1).padStart(2, "0")} / ${String(gallery.length).padStart(2, "0")}`} /><div className="detail-purchase-info"><div className="module-kicker">DETAIL SELECTION {String(index + 1).padStart(2, "0")}</div><h2>{englishValue(cleanTitle(product.catalogName || product.name), `Catalog Item ${product.id}`)}</h2><strong className="module-price">{money(product.price, product.currency)}</strong><div className="module-status"><span className="status-dot" /> {englishValue(product.stock || "Availability pending", "Availability pending")}</div>{product.colors.length > 0 && <div className="module-option-group"><span>Color / Style</span><div>{product.colors.slice(0, 8).map((color, colorIndex) => { const label = englishValue(color, `Color ${colorIndex + 1}`); return <button key={`${label}-${colorIndex}`} className={selectedColor === color ? "is-selected" : ""} onClick={() => setSelectedColor(color)}>{label}</button>; })}</div></div>}{specGroups.length > 0 && <div className="module-option-group"><span>{optionHeadingFor(product)}</span><div>{specGroups.slice(0, 10).map((size, sizeIndex) => { const label = formatOptionLabel(size, optionHeadingFor(product), sizeIndex); return <button key={`${label}-${sizeIndex}`} className={selectedSize === size ? "is-selected" : ""} onClick={() => setSelectedSize(size)}>{label}</button>; })}</div></div>}{relatedStyles.length > 1 && <div className="module-related-styles" aria-label="Other styles"><div className="module-related-head"><span>OTHER STYLES</span><small>Choose a style</small></div><div className="module-related-grid">{relatedStyles.map((item) => { const isCurrent = item.id === product.id; const itemStyle = styleLabelFor(item) || "Style option"; return <Link key={item.id} href={`/product/${item.id}`} className={`module-related-card ${isCurrent ? "is-current" : ""}`} aria-current={isCurrent ? "page" : undefined}><div className="module-related-thumb"><SafeProductImage sources={item.images} alt={`${itemStyle} preview`} loading="lazy" /></div><span><strong>{itemStyle}</strong><small>{money(item.price, item.currency)}</small></span></Link>; })}</div></div>}<a className="module-buy-button" href={primarySourceFor(product).url} target="_blank" rel="noreferrer" onClick={() => trackAffiliateClick(primarySourceFor(product), "detail_selection")}><ShoppingBag size={16} /> Buy on {primarySourceFor(product).name} <ArrowUpRight size={14} /></a>{hasPlatformSources && <div className="module-platform-list" aria-label="Other buying platforms">{platformSources.filter((source) => !source.primary).map((source) => <a key={source.name} className="module-platform-link" href={source.url} target="_blank" rel="noreferrer" onClick={() => trackAffiliateClick(source, "detail_selection_platform")}><span className="module-platform-identity"><PlatformIdentity source={source} /><span>{source.name}</span></span><ArrowUpRight size={12} /></a>)}</div>}</div></article>)}</section>
