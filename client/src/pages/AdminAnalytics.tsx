...
export default function AdminAnalytics() {
  ...
  const load = async (nextPage = eventPage, nextPageSize = eventPageSize, nextSearchPage = searchPage, nextSearchPageSize = searchPageSize) => {
    ...
    try {
      ...
      const response = await fetch(`${workerUrl}/api/admin/analytics/summary?range=${encodeURIComponent(days)}${customParams}&page=${nextPage}&pageSize=${nextPageSize}&searchPage=${nextSearchPage}&searchPageSize=${nextSearchPageSize}`, { headers: { "x-admin-api-key": key.trim() } });
      ...
      setSearchPageSize(nextSearchPageSize);
  ...
  const searchTotal = Number(summary?.searchTotal || 0);
  const searchPageCount = Math.max(1, Math.ceil(searchTotal / searchPageSize));
  const searchStart = searchTotal ? (searchPage - 1) * searchPageSize + 1 : 0;
  const searchEnd = searchTotal ? Math.min(searchPage * searchPageSize, searchTotal) : 0;
  const currentPeriodLabel = summary?.periodLabel || periodLabels[days] || "当前周期";
  const changePeriod = (value: string) => { setDays(value); setEventPage(1); setSearchPage(1); void load(1, eventPageSize, 1, searchPageSize); };
  return <div className="analytics-page"><header className="analytics-header"><Link href="/" className="admin-back"><ArrowLeft size={15} /> 返回商品目录</Link><div><span className="request-eyebrow">PRODUCTSITE 私有分析</span><h1>用户行为与商品表现</h1><p>匿名第一方事件数据，存储于 Cloudflare Worker 与 D1；不显示姓名、邮箱或原始 IP。</p></div></header><main className="analytics-main">
    ...
    {summary && <><section className="analytics-guide" aria-labelledby="analytics-guide-title"><div className="analytics-guide-heading"><span className="analytics-guide-kicker">HOW TO READ</span><h2 id="analytics-guide-title">先看懂这条用户链路</h2><p>推广链接带用户进站，用户在站内浏览商品，最后点击某个平台的购买链接。这里统计的是站内行为，<strong>不代表用户已经在第三方平台下单</strong>。</p></div><div className="analytics-guide-steps"><div><strong>1. 从哪里来</strong><span>链接里的 <code>utm_source</code> 标记来源渠道。</span></div><div><strong>2. 来了多少人</strong><span>按匿名访客去重；同一浏览器未清除站点数据时，在当前周期通常算 1 人。</span></div><div><strong>3. 看了什么</strong><span>记录商品、分类、搜索和详情页等站内行为。</span></div><div><strong>4. 点了哪里</strong><span>记录用户点击的购买平台入口，不统计平台内订单。</span></div></div></section><section className="analytics-kpis"><div><span>事件总量</span><strong>{number(summary.totals.total_events)}</strong><small>{summary.periodLabel || periodLabels[summary.range] || `最近 ${summary.days} 天`}</small></div><div><span>匿名访客</span><strong>{number(summary.totals.unique_visitors)}</strong><small>去重 anonymous ID</small></div><div><span>访问会话</span><strong>{number(summary.totals.unique_sessions)}</strong><small>去重 session ID</small></div><div><span>详情到外链点击率</span><strong>{clickRate}%</strong><small>{number(summary.totals.click_visitors)} 位点击访客 / {number(summary.totals.detail_visitors)} 位详情访客</small></div></section>
      ...
      <section className="analytics-section"><div className="analytics-section-title"><div><h2>搜索词表现</h2><p className="analytics-section-description">只统计真实搜索事件；无结果次数来自同一关键词的无结果搜索，不会把浏览列表混入搜索次数。</p></div><span>{searchTotal ? `${searchStart}-${searchEnd} / 共 ${number(searchTotal)} 个关键词` : "当前周期暂无搜索数据"}</span></div><div className="analytics-pagination-toolbar"><label>每页显示<select value={searchPageSize} onChange={(event) => void load(eventPage, eventPageSize, 1, Number(event.target.value))}><option value={10}>10 个</option><option value={25}>25 个</option><option value={50}>50 个</option><option value={100}>100 个</option></select></label><span>第 {searchPage} / {searchPageCount} 页</span><button type="button" onClick={() => void load(eventPage, eventPageSize, searchPage - 1)} disabled={loading || searchPage <= 1}>上一页</button><label className="analytics-page-jump">跳转到<input type="number" min={1} max={searchPageCount} value={searchPage} onChange={(event) => setSearchPage(Math.min(searchPageCount, Math.max(1, Number(event.target.value) || 1)))} onKeyDown={(event) => { if (event.key === "Enter") void load(eventPage, eventPageSize, searchPage); }} />页</label><button type="button" onClick={() => void load(eventPage, eventPageSize, searchPage + 1)} disabled={loading || searchPage >= searchPageCount}>下一页</button></div><div className="analytics-table-wrap"><table className="analytics-table"><thead><tr><th>搜索词</th><th>搜索次数</th><th>无结果次数</th><th>无结果占比</th></tr></thead><tbody>{summary.searches.length ? summary.searches.map((row, index) => <tr key={`${row.name}-${index}`}><td><strong>{row.name || "—"}</strong></td><td>{number(row.count)}</td><td>{number(row.no_result_count)}</td><td>{((Number(row.no_result_count || 0) / Math.max(1, Number(row.count || 0))) * 100).toFixed(1)}%</td></tr>) : <tr><td colSpan={4}>当前周期暂无搜索数据。</td></tr>}</tbody></table></div></section>
