...
export default function AdminAnalytics() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) throw new Error("Invalid admin key. Check the ADMIN_API_KEY configured on the Worker.");
        if (response.status === 404) throw new Error("Analytics endpoint is not deployed on the Worker yet.");
        if (response.status >= 500) throw new Error(payload.error || "Worker or D1 is unavailable. Run the analytics_events migration first.");
        throw new Error(payload.error || "Unable to load analytics.");
      }
      setSummary(payload as Summary);
    } catch (error) {
      setMessage(error instanceof TypeError ? "Unable to reach the Worker. Check VITE_CLOUDFLARE_WORKER_URL and the Worker CORS origin." : error instanceof Error ? error.message : "Unable to load analytics.");
