...
function Ranking({ title, rows, labelKey = "name", empty = "当前周期暂无数据", periodLabel = "当前周期", periodValue = "30", onPeriodChange, rowAction }: { title: string; rows: Row[]; labelKey?: "name" | "id"; empty?: string; periodLabel?: string; periodValue?: string; onPeriodChange?: (value: string) => void; rowAction?: (row: Row) => ReactNode }) {
  const max = Math.max(1, ...rows.map((row) => Number(row.count || 0)));
  return <section className="analytics-section"><div className="analytics-section-title"><h2>{title}</h2><span><select className="analytics-card-period" value={periodValue} onChange={(event) => onPeriodChange?.(event.target.value)} aria-label={`${title}统计周期`}>{periodOptions.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select> · 前 10 名</span></div>{rows.length ? <div className="analytics-ranking">{rows.slice(0, 10).map((row, index) => <div className="analytics-rank" key={`${row[labelKey] || "unknown"}-${index}`}><div className="analytics-rank-label"><strong>{index + 1}. {label(String(row[labelKey] || "未标注"))}</strong><span className="analytics-rank-count">{number(row.count)}</span>{rowAction?.(row)}</div><div className="analytics-bar"><i style={{ width: `${Math.max(4, (Number(row.count || 0) / max) * 100)}%` }} /></div></div>)}</div> : <p className="analytics-muted">{empty}</p>}</section>;
}
