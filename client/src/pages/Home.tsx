  ...
  return <div className="catalog-shell">
    <aside className="catalog-rail">
      ...
      <nav className="category-nav" aria-label="商品大类">
        {navItems.map((item) => <button key={item.id} className={`category-link ${category === item.id ? "is-active" : ""}`} onClick={() => { setCategory(item.id); setBrand("all"); }}><span>{item.label}</span><small>{String(item.count).padStart(2, "0")}</small></button>)}
      </nav>
      <div className="rail-rule brand-rule" /><div className="rail-kicker brand-kicker">BRANDS <span>{brandItems.length - 1}+</span></div>
      <nav className="brand-nav" aria-label="品牌选择">
        {brandItems.map((item) => <button key={item.id} className={`brand-link ${brand === item.id ? "is-active" : ""}`} onClick={() => setBrand(item.id)}><span>{item.label}</span><small>{String(item.count).padStart(2, "0")}</small></button>)}
      </nav>
      <div className="rail-footer"><span>CATALOG / 01</span><span>2026</span></div>
    </aside>
    <main className="catalog-main">
      <header className="catalog-header"><div className="mobile-brand"><img src="/manus-storage/catalog-mark_f15a35f4.png" alt="" /> <span>Material Catalog</span></div><div className="search-wrap"><Search size={17} strokeWidth={1.8} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="搜索品牌、商品或品类" aria-label="搜索商品" />{query && <button onClick={() => setQuery("")}>清除</button>}</div><div className="header-actions"><button className="header-icon" aria-label="筛选"><SlidersHorizontal size={18} /></button><button className="header-icon" aria-label="网格视图"><Grid2X2 size={18} /></button><span className="profile-dot">M</span></div></header>
      <section className="catalog-intro"><div><div className="eyebrow"><Sparkles size={13} /> EDITED GOODS / 01</div><h1>Find the next<br /><i>piece.</i></h1></div><div className="intro-note">精选服饰与生活方式单品。<br />点击图片，打开商品档案。</div></section>
      ...
      {visible.length > 0 ? <section className="masonry-grid" aria-label="商品列表">{visible.map((product, index) => { const image = product.images[0]; const isFav = favorites.includes(product.id); return <article className={`product-card card-${index % 7}`} key={product.id} onClick={() => navigate(`/product/${product.id}`)}><div className="product-image-wrap"><img src={image} alt={product.catalogName || product.name} loading={index < 8 ? "eager" : "lazy"} /><div className="image-wash" /><button className={`favorite-button ${isFav ? "is-favorite" : ""}`} onClick={(e) => { e.stopPropagation(); toggleFavorite(product.id); }} aria-label={isFav ? "取消收藏" : "收藏商品"}><Heart size={16} fill={isFav ? "currentColor" : "none"} /></button><span className="view-stamp">VIEW FILE <ArrowUpRight size={14} /></span></div><div className="product-meta"><div className="product-name">{cleanTitle(product.catalogName || product.name)}</div><div className="product-sub"><span>{product.brand || product.subCategory || "CATALOG ITEM"}</span><strong>{money(product.price, product.currency)}</strong></div></div></article>; })}</section> : <div className="empty-state"><span>NO MATCHES / 00</span><h2>换一个关键词试试。</h2><button onClick={resetFilters}>清除筛选</button></div>}
    </main>
  </div>;
}
