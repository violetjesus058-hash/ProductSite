...
const UNBRANDED_BRAND = "__accessories__";
const navItems = [
...
export default function Home() {
  ...
  const visible = useMemo(() => {
    ...
    const filtered = products.filter((product) => {
      ...
      const matchesBrand = brand === "all" || (brand === UNBRANDED_BRAND ? !product.brand : product.brand === brand);
  ...
  const mobileBrandItems = useMemo(() => { const names = Array.from(new Set(products.map((product) => product.brand).filter(Boolean))).sort((a, b) => a.localeCompare(b)); return [{ id: "all", label: "ALL" }, ...names.map((name) => ({ id: name, label: name })), { id: UNBRANDED_BRAND, label: "ACCESSORIES" }]; }, []);
  const subCategoryItems = useMemo(() => { const scoped = products.filter((product) => brand === "all" || (brand === UNBRANDED_BRAND ? !product.brand : product.brand === brand)); const values = Array.from(new Set(scoped.map((product) => product.subCategory).filter(Boolean))).sort((a, b) => a.localeCompare(b)); return ["all", ...values]; }, [brand]);
