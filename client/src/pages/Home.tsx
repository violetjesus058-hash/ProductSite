...
export default function Home() {
  ...
  const [dislikes, setDislikes] = useState<string[]>(() => readDislikes());
  ...
  const categoryRecommendationPool = useMemo(() => {
    ...
    const pool = buildCategoryRecommendationPool(category, products, excluded);
    return rankBehavioralRecommendations(pool, products, new Set(favorites), engagement, Date.now(), new Set(dislikes));
  }, [category, visible, favorites, engagement, dislikes]);
  ...
  return <div className={`catalog-shell catalog-style-${pageStyle} type-size-${fontSizeLevel} tracking-level-${letterSpacingLevel}`}>
    ...
    <main className="catalog-main">
      <header className="catalog-header"><div className="mobile-brand"><img src="/branding/rivora-icon.png" alt="RIVORA Apparel & Lifestyle" className="mobile-brand-icon" /></div><div className="mobile-header-center"><div className="search-wrap"><Search size={17} strokeWidth={1.8} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search brands, products, or categories" aria-label="Search products" />{query && <button onClick={() => setQuery("")}>Clear</button>}</div><div className="mobile-status-row"><span><i /> Browsing {demoViewers}</span><span>Historical visits 20K+</span></div></div><div className="header-actions"><div className="header-menu"><button className={`header-icon header-record-button ${openPanel === "favorites" ? "is-active" : ""}`} aria-label={`Saved items, ${favoriteProducts.length}`} aria-expanded={openPanel === "favorites"} onClick={() => setOpenPanel(openPanel === "favorites" ? null : "favorites")}><Heart size={18} fill={favoriteProducts.length ? "currentColor" : "none"} /><span>{favoriteProducts.length}</span></button>{openPanel === "favorites" && <div className="record-popover"><div className="record-popover-head"><strong>SAVED ITEMS</strong><div className="record-popover-actions">{favoriteProducts.length > 0 && <button className="record-clear-button" onClick={clearFavorites} aria-label="Clear all saved items">Clear all</button>}<button onClick={() => setOpenPanel(null)} aria-label="Close saved items"><X size={15} /></button></div></div>{favoriteProducts.length ? favoriteProducts.map((product) => <button className="record-row" key={product.id} onClick={() => openProduct(product.id)}><SafeProductImage sources={product.images} alt="" /><span><strong>{cleanTitle(product.catalogName || product.name)}</strong><small>{money(product.price, product.currency)}</small></span><ArrowUpRight size={14} /></button>) : <p className="record-empty">Your saved products will appear here.</p>}</div>}</div><div className="header-menu"><button className={`header-icon header-record-button ${openPanel === "history" ? "is-active" : ""}`} aria-label={`Browsing history, ${historyProducts.length}`} aria-expanded={openPanel === "history"} onClick={() => { setHistory(readHistory()); setOpenPanel(openPanel === "history" ? null : "history"); }}><HistoryIcon size={18} /><span>{historyProducts.length}</span></button>{openPanel === "history" && <div className="record-popover"><div className="record-popover-head"><strong>BROWSING HISTORY</strong><button onClick={() => setOpenPanel(null)} aria-label="Close browsing history"><X size={15} /></button></div>{historyProducts.length ? historyProducts.map(({ entry, product }) => <button className="record-row" key={product.id} onClick={() => openProduct(product.id)}><SafeProductImage sources={product.images} alt="" /><span><strong>{cleanTitle(product.catalogName || product.name)}</strong><small>Viewed {formatVisitTime(entry.visitedAt)}</small></span><ArrowUpRight size={14} /></button>) : <p className="record-empty">Products you open will appear here.</p>}</div>}</div><span className="live-status"><i /> <span className="demo-label">Demo</span> · Browsing {demoViewers} · Historical visits 20K+</span></div></header>

      {openPanel === "favorites" && <section className="mobile-favorites-view" aria-labelledby="mobile-favorites-title"><div className="mobile-favorites-head"><div><span className="recommendation-kicker">YOUR SELECTION</span><h2 id="mobile-favorites-title">Saved products</h2><p>{favoriteProducts.length ? `${favoriteProducts.length} saved product${favoriteProducts.length === 1 ? "" : "s"}` : "Products you save will appear here."}</p></div><div className="mobile-favorites-actions">{favoriteProducts.length > 0 && <button className="record-clear-button" onClick={clearFavorites}>Clear all</button>}<button className="mobile-favorites-close" onClick={() => setOpenPanel(null)} aria-label="Close saved products"><X size={16} /></button></div></div>{favoriteProducts.length > 0 ? <div className="mobile-favorites-grid">{favoriteProducts.map((product) => { const savedTitle = englishValue(cleanTitle(product.catalogName || product.name), `Catalog Item ${product.id}`); const savedDisliked = dislikes.includes(product.id); return <article className="mobile-favorite-card" key={product.id} onClick={() => openProduct(product.id)}><div className="mobile-favorite-image"><SafeProductImage sources={product.images} alt={savedTitle} /><button className="favorite-button is-favorite" onClick={(event) => { event.stopPropagation(); toggleFavorite(product.id); }} aria-label="Remove from saved items"><Heart size={15} fill="currentColor" /></button><button className={`dislike-button ${savedDisliked ? "is-disliked" : ""}`} onClick={(event) => { event.stopPropagation(); toggleProductDislike(product.id); }} aria-label="Not interested in this product"><ThumbsDown size={12} /></button></div><h3>{savedTitle}</h3><div className="product-price">{money(product.price, "USD")} <span className="product-currency">USD</span></div></article>; })}</div> : <div className="mobile-favorites-empty">Save a product with the heart button to build your selection.</div>}<div className="mobile-favorites-recommendation"><div className="mobile-favorites-recommendation-head"><div><span className="recommendation-kicker">KEEP EXPLORING</span><h2>Recommended for you</h2><p>More pieces selected from your browsing activity.</p></div></div>{favoriteRecommendations.length > 0 && <div className="mobile-favorites-grid is-recommendation">{favoriteRecommendations.map((product) => { const recommendationTitle = englishValue(cleanTitle(product.catalogName || product.name), `Catalog Item ${product.id}`); const recommendationDisliked = dislikes.includes(product.id); return <article className="mobile-favorite-card" key={`saved-recommendation-${product.id}`} onClick={() => openProduct(product.id)}><div className="mobile-favorite-image"><SafeProductImage sources={product.images} alt={recommendationTitle} /><button className="favorite-button" onClick={(event) => { event.stopPropagation(); toggleFavorite(product.id); }} aria-label="Save product"><Heart size={15} /></button><button className={`dislike-button ${recommendationDisliked ? "is-disliked" : ""}`} onClick={(event) => { event.stopPropagation(); toggleProductDislike(product.id); }} aria-label="Not interested in this product"><ThumbsDown size={12} /></button></div><h3>{recommendationTitle}</h3><div className="product-price">{money(product.price, "USD")} <span className="product-currency">USD</span></div></article>; })}</div>}</div></section>}
