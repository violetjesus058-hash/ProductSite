...
const CUSTOM_SOURCE = "__custom_url__";
function parseSourceUrl(value: string) { try { const url = new URL(value.trim()); if (!["http:", "https:"].includes(url.protocol) || !url.hostname) return null; return url; } catch { return null; } }
...
export default function PromotionLinkBuilder() {
  ...
  const generatedUrl = useMemo(() => {
    ...
    const customUrl = source === CUSTOM_SOURCE ? parseSourceUrl(customSourceUrl) : null;
    const sourceValue = customUrl ? customUrl.hostname.replace(/^www\./, "") : source;
    const sourceTag = customUrl ? `${content.trim()}-ref-${customUrl.hostname.replace(/^www\./, "").replace(/[^a-z0-9.-]+/gi, "-")}` : content;
    const values = { utm_source: sourceValue, utm_medium: medium, utm_campaign: campaign, utm_content: sourceTag };
    Object.entries(values).forEach(([name, value]) => { if (value.trim()) url.searchParams.set(name, value.trim().toLowerCase().replace(/\s+/g, "-")); });
    return url.toString();
  }, [campaign, content, customSourceUrl, medium, productId, source, target]);
  const customSourceError = source === CUSTOM_SOURCE && customSourceUrl.trim() && !parseSourceUrl(customSourceUrl) ? "请输入完整的 http:// 或 https:// 网址" : "";
